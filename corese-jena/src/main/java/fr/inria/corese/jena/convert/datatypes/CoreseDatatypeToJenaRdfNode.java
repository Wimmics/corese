...
/**
 * Converts Corese datatypes to equivalent Jena RDFNodes.
 *
 * This class provides a public static method 'convert' that takes a Corese library
 * 'IDatatype' object and returns an equivalent 'RDFNode' object from the Jena
 * library. The conversion is done through a series of private methods that
 * handle different types of Corese datatypes, such as URI, blank node, literal,
 * lang string, and various numerical and boolean types. The code uses Jena's
 * 'ModelFactory' to create a default model for creating Jena RDFNodes, and it
 * uses various classes from the Jena datatypes and RDF packages for converting
 * the Corese datatypes.
 *
