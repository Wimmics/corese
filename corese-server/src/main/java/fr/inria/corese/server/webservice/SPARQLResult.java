...
public class SPARQLResult implements ResultFormatDef, URLParam {
     ...
     * Content-Type is set according to format parameter and what is returned by
     * ResultFormat
     * 
     * @name is a) the name of a specific triple store, b) undefined for standard
     *       sparql endpoint
     ...
     * @format is json|xml to specify return format when there is no http header
     *         content
     * @type is format specified by content negotiation http header (consider type
     *       otherwise format)
     ...
     * For *strong* SPARQL compliance, use dataset.complete() before returning the
     * dataset.
     *
     * @return a dataset
     */
     ...
     * Return either
     * a) when mode=link : query result with link url to transformation result
     * document
    ...
    ResultFormat getFormatTransform(Mappings map, Dataset ds, String format, int type, List<String> transformList) {
        ...
        if (link) {
            ...
            // record url of std result document in case transform generate link to std
            // result (cf mapper)
    ...
    Optional<ResultFormat> getFormatTransformList(Mappings map, Dataset ds, String format, int type,
        ...
        for (String transform : transformList) {
            ...
            if (c.hasAnyValue(LINK, LINK_REST)) {
                // mode=link
                // save transformation result in document and record URL of document in map
                // result link
        ...
        if (c.hasValue(LINK_REST)) {
            // return result of first transformation (it may have generated links to other
            // transformations)
     ...
     * Each LinkedResult will be used to store a result in a document accessible by
     * URL
