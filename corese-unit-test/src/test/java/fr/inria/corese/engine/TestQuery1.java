...
public class TestQuery1 {
    ...
    static void before() {
        ...
        String q = "@public {" + "@error function us:error(?e, ?x , ?y) { " + "xt:print('****************** error') ; "
    ...
    static void before2() {
        ...
        String q = "@public {"
                ...
                + "@error " + "function us:error(?e, ?x , ?y) { " + "us:recerror(?e, ?x, ?y) ; " + "error() " + "}"

                + "@filter " + "function us:filter(?g, ?e, ?b) { "
                // + "xt:print(?e);"
                + "us:record(?e) ;" + "?b " + "}"

                + "@select " + "function us:select(?e, ?b) { "
                // + "xt:print(?e);"
                + "us:record(?e) ;" + "?b " + "}"

                + "function us:map() { ds:getPublicDatatypeValue(true) }"

                + "function us:record(?e) {" + "if (java:isTerm(?e)) {"
                + "xt:set(us:map(), java:getLabel(?e), coalesce(1 + xt:get(us:map(), java:getLabel(?e)), 1)) ;"
                + "let (( | ?l) = ?e) {" + "for (?ee in ?e) {" + "us:record(?ee)" + "}" + "}" + "}" + "}"

                + "function us:recerror(?e, ?x, ?y) {" + "xt:add(xt:get(us:map(), us:error), xt:list(?e, ?x, ?y))" + "}"
    ...
    public static Graph graph() {
        ...
        Load ld = Load.create(graph);
        try {
            ld.parse(Thread.currentThread().getContextClassLoader().getResource("data").getPath() + "/"
                    + "comma/comma.rdfs");
            ld.parse(Thread.currentThread().getContextClassLoader().getResource("data").getPath() + "/"
                    + "comma/model.rdf");
            ld.parseDir(
                    Thread.currentThread().getContextClassLoader().getResource("data").getPath() + "/" + "comma/data");
        } catch (LoadException ex) {
    ...
    public void format() throws EngineException {
        String i = "insert data { [] rdf:value () }";
        String q = "select (xt:xml(?m) as ?xml) (xt:rdf(?m) as ?rdf) (xt:json(?m) as ?json) " + "(xt:graph(?m) as ?g) "
                + "where { bind (us:test() as ?m) }"

                + "function us:test() { " + "query(select * where {?s ?p ?o})" + "}";
    ...
    public void testmap2() throws EngineException {
        String q = "select (us:test(dt:map) as ?t)  " + "(us:test(dt:json) as ?js)" + "where {}"

                + "function us:test(atype) {" + "let (amap = xt:create(atype), " + "list = @(('a' 1)('b' 2)('c' 3))) {"
                + "for ((key val) in list) {" + "xt:set(amap, key, val)" + "} ;" + "let ((aa | bb . cc) = amap,"
                + "(ee) = bb) {" + "reduce(rq:plus, maplist (function((key, val)) { val }, xt:list(aa, ee, cc)))" + "}"
                + "}" + "}";
    ...
    public void testmap() throws EngineException {
        String q = "select (us:test(dt:map) as ?t)  " + "(us:test(dt:json) as ?js)" + "where {}"
                ...
                + "function us:test(atype) {" + "let (amap = xt:create(atype), " + "list = @(('a' 1)('b' 2)('c' 3))) {"
                + "for ((key val) in list) {" + "xt:set(amap, key, val)" + "} ;"
    ...
    public void testgraph5() throws EngineException {
        String q = "select (us:test() as ?t)  " + "where {}"
                ...
                + "function us:test() {" + "let (agraph = construct {[] rdf:value 1, 2, 3} where {}) {"
    ...
    public void exception2() {
        String q = "select ?x (us:test() as ?t)  where { values ?x {1} }"
                + "function us:test() { try {us:foo() } catch (var) { var }  }" + "function us:foo()  {throw(1)}";
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        try {
            Mappings map = exec.query(q);
    ...
    public void exception() throws EngineException {
        String q = "select ?x (us:test() as ?t)  where { values ?x {1} }" + "function us:test() { throw(1) }";
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        try {
            Mappings map = exec.query(q);
    ...
    public void testStatic2() throws EngineException {
        String q = "select (us:test() as ?t)  where {}" + "function us:test() {static (stat = stat+1) }";
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
    ...
    public void testStatic() throws EngineException {
        String q = "select (us:test() as ?t)  where {}" + "function us:test() {let (stat = stat+1) {return (stat) }}";
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        Binding.setStaticVariable("?stat", DatatypeMap.ONE);
        Mappings map = exec.query(q);
    ...
    public void testForLoop() throws EngineException, LoadException, IOException {
        ...
        String q = "select (us:fun() as ?list) where {" + "}" + "function us:fun() {" + "let (list = xt:list()) {"
                + "for ((s, p, o) in select * where {?s ?p ?o}) {" + "xt:add(list, xt:list(s, p, o))" + "};"
                + "return (list)" + "} " + "}";
    ...
    public void testXML4() throws EngineException, LoadException, IOException {
        ...
        String q = "select " + " ?x ?y ?z " + "where {" + "bind ('"
                ...
                + "<test rdf:datatype=\"http://www.w3.org/2001/XMLSchema#boolean\">true</test>" + "</doc>' as ?doc)"
                + "bind (xt:xml(?doc) as ?xml)" + "bind (xt:list(us:test(?xml)) as ?test)"
                + "values (?x ?y ?z) {unnest(?test)}" + "}"

                + "function us:test(xml) {" + "let (list = xt:xpath(xml,'/doc/*/text()')" + ") {"
    ...
    public void testXML3() throws EngineException, LoadException, IOException {
        ...
        String q = "select " + " ?x ?y ?z " + "where {" + "bind ('"
                ...
                + "<ns:test ns:att=\"att\">text</ns:test>" + "<other>" + "<![CDATA[cdata text]]>"
                + "<!--comment text-->" + "<?proc proc text?>" + "</other>" + "</doc>' as ?doc)"
                + "bind (xt:xml(?doc) as ?xml)" + "bind (xt:list(us:test(?xml)) as ?test)"
                + "values (?x ?y ?z) {unnest(?test)}" + "}"

                + "function us:test(xml) {" + "let ((other) = dom:getElementsByTagName(xml,  \"other\"), "
                // + "sublist = dom:getChildNodes(other)"
                + "sublist = xt:xpath(xml,'//other/node()')" + ") {" + "maplist(dom:getNodeValue, sublist)" + "}" + "}";
    ...
    public void testXML2() throws EngineException, LoadException {
        ...
        String q = "select " + "(us:fun(?xml) as ?t) " + "(dom:getNamespaceURI(?elem) as ?ns)"
                + "(dom:getBaseURI(?elem) as ?base)" + "(dom:getNodeName(?elem) as ?name)"
                + "(dom:getLocalName(?elem) as ?local)" + "(dom:hasAttributeNS(?elem, ?ns, 'att') as ?b)"
                + "(dom:getAttributeNS(?elem, ?ns, 'att') as ?att)" + "(dom:getFirstChild(?elem) as ?child)"
                + "(dom:getTextContent(?child) as ?text)" + "where {"
                + "bind (xt:xml('<doc xmlns:ns=\"http://example.org/\"  xml:base=\"http://example.org/\">"
                + "<ns:test ns:att=\"att\">text</ns:test></doc>') as ?xml)" + "bind (us:fun(?xml) as ?elem)" + "}"

                + "function us:fun(xml) {" + "let (dom = dom:getFirstChild(xml), ns = \"http://example.org/\","
                + "list = dom:getElementsByTagNameNS(dom, ns, \"test\")," + "(first | rest) = list" + ") {" + "first"
                + "}" + "}";
    ...
    public void testXML1() throws EngineException {
        ...
        String init = "insert data {" + "<doc> us:contain "
        ...
        String query = "select (xt:text(?dom) as ?t) " + "(xt:get(xt:attributes(?dom), 'color') as ?c)"
                + "(xt:nodename(?dom) as ?n)" + "where { " + "?x us:contain ?xml " + "bind (xt:xml(?xml) as ?doc)"
                + "bind (us:phrase(?doc) as ?dom)" + "}"

                + "function us:phrase(root) {" + "for (doc in root) {" + "for (phrase in xt:elements(doc, 'phrase')) {"
                + "return (phrase)" + "}" + "}" + "}";
    ...
    public void testReduce() throws EngineException, LoadException {
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String q = "select (reduce(us:test, xt:list()) as ?x) " + "(reduce(us:test, xt:list(1)) as ?y)"
                + "(reduce(us:test, xt:list(1, 2)) as ?z)" + "where {}"

                + "function us:test(x, y) {" + "x + y" + "}"
    ...
    public void testDynLet() throws EngineException, LoadException {
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String q = "select (us:test(10) as ?l) (xt:get(?l, 0) as ?n) where {}" + "" + "function us:test(x) { "
    ...
    public void testDynLet3() throws EngineException, LoadException {
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String q = "select (us:test() as ?n) where {}" + "" + "function us:test() { " + "letdyn (x = 1) { us:fun() } "
                + "}"
    ...
    public void testDynLet4() throws EngineException, LoadException {
        ...
        String q = "select (reduce(rq:plus, us:bar(10)) as ?a) where {}"
                ...
                + "function us:bar(n) {" + "letdyn(select (1 as ?x) where {}) {"
    ...
    public void testDynLet44() throws EngineException, LoadException {
        ...
        String q = "select (reduce(rq:plus, us:bar(10)) as ?a) where {}"
                ...
                + "function us:bar(n) {" + "letdyn(select (1 as ?x) where {}) {" + "maplist(us:test, xt:iota(n)) " + "}"
                + "}"

                + "function us:test(y) {" + "set(x = 2*x)" + "}";
        Mappings map = exec.query(q);
        assertEquals(2046, map.getValue("?a").intValue());
    }
    ...
    public void testDynLet5() throws EngineException, LoadException {
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String q = "select (us:test() as ?n) where {}" + "" + "function us:test() { " + "let (z = 1) {"
                + "letdyn (x = 1) { us:fun() } " + "}" + "}"
    ...
    public void testexist() throws EngineException, LoadException {
        ...
        String i = "insert data {  " + "graph us:g1 { " + "rdf:type rdf:type   rdf:Property "
                + "rdf:type owl:sameAs rdf:type " + "rdf:type us:test  us:test " + "us:test us:test us:test " + "}"
                + "}";

        String q = "select * where {" + "bind (bnode() as ?bn)"
                ...
                + "graph kg:default {" + "bind (bnode() as ?bb)" + "bind (xt:exists(?bb,  ?bb, rdf:Property) as ?a1)"
    ...
    public void testTTL1() throws EngineException, LoadException {
        ...
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String i = "insert data {  " + "_:b rdf:value (_:b)  " + "}";
    ...
    public void testTTL3() throws EngineException, LoadException {
        ...
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String i = "insert data {  " + "_:b rdf:value [rdf:value _:b]  " + "}";
        String q = "select (st:apply-templates-with(st:turtle) as ?t) where { }";
        ...
        QueryProcess ex = QueryProcess.create(gg);
        String qq = "select * where {" + "?s ?p [ rdf:value ?s]  " + "}";
    ...
    public void testTTL4() throws EngineException, LoadException {
        ...
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String i = "insert data {  " + "_:a rdf:value _:e " + "_:b rdf:value _:e " + "_:e rdf:value _:b " + "}";
        ...
        QueryProcess ex = QueryProcess.create(gg);
        String qq = "select * where {" + "?a rdf:value ?e " + "?b rdf:value ?e " + "?e rdf:value ?b   " + "}";
    ...
    public void testNGG1() throws EngineException {
        ...
        String i = "insert data {" + "graph us:g1 { us:John foaf:knows us:Jim }"
                + "graph us:g2 { us:Jim foaf:knows us:Jack . us:Jack foaf:knows us:James }" + "}";

        String q = "select * where {" + "graph ?g {" + "?s foaf:knows  ?o " + "optional { ?o foaf:knows ?y }" + "}"
                + "}";
    ...
    public void testNGG2() throws EngineException {
        ...
        String i = "insert data {" + "graph us:g1 { us:John foaf:knows us:Jim, us:Jesse }"
                + "graph us:g2 { us:Jim foaf:knows us:Jack . us:Jack foaf:knows us:James }" + "}";

        String q = "select * where {" + "graph ?g {" + "?s foaf:knows  ?o " + "minus { ?o foaf:knows ?y }" + "}" + "}";
    ...
    public void testNGG3() throws EngineException {
        ...
        String q = "select * where {" + "graph ?g {" + "{?s foaf:knows  ?o }"
    ...
    public void testNGG4() throws EngineException {
        ...
        String i = "insert data {" + "graph us:g1 { us:Jack foaf:knows us:Jim, us:Jesse }"
                + "graph us:g2 { us:Jim foaf:knows us:Jack . us:Jules foaf:knows us:James }" + "}";

        String q = "select * where {" + "graph ?g {" + "?s foaf:knows  ?o filter exists {?o foaf:knows ?y} " + "}"
                + "}";
    ...
    public void testNGG5() throws EngineException {
        ...
        String q = "select * " + "from named us:g2 " + "where {" + "graph ?g {" + "?s foaf:knows  ?o "
    ...
    public void testNGG6() throws EngineException {
        ...
        String q = "select * " + "where {" + "graph us:g2 {" + "?s foaf:knows  ?o " + "optional { ?o foaf:knows ?y }"
                + "}" + "}";
    ...
    public void testNGG7() throws EngineException {
        ...
        String i = "insert data {" + "graph us:g1 { us:John foaf:knows us:Jim . us:Jack foaf:knows us:James}"
                + "graph us:g2 { us:Jim foaf:knows us:Jack . us:Jack foaf:knows us:James }" + "}";

        String q = "select * " + "from named us:g1 " + "from named us:g2 " + "where {" + "graph ?g {"
    ...
    public void testNGG8() throws EngineException {
        ...
        String i = "insert data {" + "graph us:g1 { us:John foaf:knows us:Jim . us:Jack foaf:knows us:Jim}"
                + "graph us:g2 { us:Jim foaf:knows us:Jack . us:Jack foaf:knows us:James }" + "}";

        String q = "select * " + "from named us:g1 " + "from named us:g2 " + "where {" + "graph ?g {"
                + "?s foaf:knows  ?o " + "{select * where  { ?o foaf:knows ?y }}" + "}" + "}";
    ...
    public void testNGG9() throws EngineException {
        ...
        String q = "select * " + "from named us:g1 " + "from named us:g2 " + "where {" + "graph ?g {"
                + "?s foaf:knows  ?o " + "bind ( exists { ?o foaf:knows ?y } as ?b )" + "}" + "}";
    ...
    public void testNGG10() throws EngineException {
        ...
        String q = "select * " + "from named us:g1 " + "from named us:g2 " + "where {" + "graph ?g {"
                + "?s foaf:knows  ?o " + "values ?b { unnest( exists { ?o foaf:knows ?y } )  }" + "}" + "}";
    ...
    public void testNGG11() throws EngineException {
        ...
        String q = "select * where {"
                ...
                + "graph ?g { " + "{ ?s foaf:knows ?o filter exists {?s foaf:knows ?t} graph ?g {filter us:test(?g)} "
                + "  optional { ?o foaf:knows ?y filter exists {?y foaf:knows ?t}}}" + "union"
    ...
    public void testRee() throws EngineException {
        ...
        String i2 = "with <http://example.org/g> " + "insert  { ?x foaf:name ?n } where { ?x rdfs:label ?n }";

        String q = "select * " + "from  <http://example.org/g> " + "where { ?x ?p ?y  }";

        String q2 = "select * " + "from  <http://example.org/g> " + "where { ?x foaf:name ?y  }";

        String q4 = "select * " + "from named <http://example.org/g> " + "where { " + "graph ?g { ?x foaf:name ?y  } "
                + "}";
    ...
    public void testLock() throws EngineException {
        ...
        String q3 = "select * where {" + "graph us:g1 { ?s ?p ?o }" + "}";
    ...
    public void testBB() throws EngineException {
        ...
        String i = "insert data {" + "us:John foaf:knows us:Jack ." + "us:Jim  foaf:knows us:James ." + "}";

        String q = "select * where {" + "?a foaf:knows  ?b " + "{select * where  { ?a foaf:knows ?c }}"
    ...
    public void testUpdate() throws EngineException {
        ...
        String q = "select (us:foo(us:John) as ?g) where {}"
                ...
                + "function us:foo(?x) {" + "let (?g = construct {} where {} ) {" + "xt:focus(?g, query("
                + "insert data { " + "us:John rdfs:label 'John', 'Jack'" + "us:Jim  rdfs:label 'Jim'" + "} ;"
                + "delete {?x rdfs:label ?name} insert {?x foaf:name ?name} " + "where  {?x rdfs:label ?name}" + ") ) ;"
                + "?g" + "}" + "}";
        ...
        String qq = "select ?p (count(*) as ?c) {" + "?x ?p ?y" + "}" + "group by ?p order by ?c ";
    ...
    public void testMap3() throws EngineException, LoadException {
        ...
        String q = "select  " + "(xt:size(us:test(5))   as ?a)" + "(xt:size(us:test(10))  as ?b)"
                + "(xt:size(us:test(15))  as ?c)" + "(xt:size(us:test2(5))  as ?d)" + "(xt:size(us:test2(10)) as ?e)"
                + "(xt:size(us:test2(15)) as ?f)" + "where {}"

                + "function us:test(?n) {" + "maplist(lambda(?x, ?y) { xt:list(?x, ?y) }, xt:iota(?n), us:map())" + "}"

                + "function us:test2(?n) {" + "maplist(lambda(?x, ?y) { xt:list(?x, ?y) }, us:map(),xt:iota(?n))" + "}"

                + "function us:map() {" + "let (?m = xt:map()) {" + "xt:set(?m, 1, 1) ;" + "xt:set(?m, 01, 01) ;"
                + "xt:set(?m, 1.0, 1.0) ;" + "xt:set(?m, 1.0e0, 1.0e0) ;" + "xt:set(?m, '1'^^xsd:int, '1'^^xsd:int) ;"
                + "xt:set(?m, 1, 1) ;" + "xt:set(?m, 01, 01) ;" + "xt:set(?m, true, true) ;"
                + "xt:set(?m, st:test, st:test) ;" + "xt:set(?m, 'test', 'test') ;"
                + "xt:set(?m, 'test'@fr, 'test'@fr) ;" + "xt:set(?m, 'test', 'test') ;"
                + "xt:set(?m, bnode(), bnode()) ;" + "return (?m)" + "}" + "}";
    ...
    public void testMap2() throws EngineException, LoadException {
        ...
        String q = "function xt:main() {" + "let (?m = xt:map()) {" + "xt:set(?m, 1, 1) ;" + "xt:set(?m, 01, 01) ;"
                + "xt:set(?m, 1.0, 1.0) ;" + "xt:set(?m, 1.0e0, 1.0e0) ;" + "xt:set(?m, '1'^^xsd:int, '1'^^xsd:int) ;"
                + "xt:set(?m, 1, 1) ;" + "xt:set(?m, 01, 01) ;" + "xt:set(?m, true, true) ;"
                + "xt:set(?m, st:test, st:test) ;" + "xt:set(?m, 'test', 'test') ;"
                + "xt:set(?m, 'test'@fr, 'test'@fr) ;" + "xt:set(?m, 'test', 'test') ;"
    ...
    public void testNGExist() throws EngineException, LoadException {
        ...
        String i = "insert data {" + "graph us:g1 { us:Jack foaf:name 'Jack' ; foaf:knows us:Jim }"
                + "graph us:g2 { us:Jack foaf:knows us:James . us:James foaf:name 'James' }" + "}";

        String q = "select * " + "where {" + "graph ?g {  "
    ...
    public void testNG() throws EngineException, LoadException {
        ...
        String q = "select * " + "where {" + "graph ?g {  ?x foaf:name ?n  "
    ...
    public void testPPmatch2() throws EngineException {
        String q = "function xt:main() { us:test(xt:iota(2)) }"
                ...
                + "function us:test(?list) {" + "let ((?a ?b | ?l . ?c ?d) = ?list) {"
    ...
    public void testpplist() throws LoadException, EngineException {
        ...
        String i = "insert data { " + "graph us:g1 { us:John us:age 10 } " + "graph us:g2 { us:John us:age 10, 20, 30 }"
                + "}";

        String q = "prefix ns: <http://example.org>" + "select * " + "(let ((?x ?y | ?l) = xt:graph()) { ?l } as ?l) "
                + "(let ((?t | ?ll) = ?l) { ?t } as ?t) " + "(let ((?s ?p | ?ll) = ?t) { ?ll } as ?ll) "
                + "(let ((?o | ?rst) = ?ll) { ?o } as ?o) " + "where {" + "}";
        exec.query(i);
        Mappings map = exec.query(q);
        // System.out.println(map);
    ...
    public void testplist() throws LoadException, EngineException {
        ...
        String q = "prefix ns: <http://example.org>" + "select * "
                + "(let ((?x ?y | ?l) = xt:list(1, 2, 3, 4)) { ?l } as ?l) "
                + "(let ((?x ?y | ?l) = xt:list(1, 2, 3, 4)) { ?y } as ?y) " + "where {" + "}";
        Mappings map = exec.query(q);
    ...
    public void testextequal() throws EngineException {
        QueryProcess exec = QueryProcess.create(Graph.create());
        String q = "select " + "(kg:equals('ete', 'été') as ?eq)" + "(kg:contains('un été', 'ete') as ?ct)"
                + "where {}";
        Mappings map = exec.query(q);
    ...
    public void testgraphit() throws LoadException, EngineException {
        ...
        String i = "insert data { " + "graph us:g1 { us:John us:age 10, 20 } "
                + "graph us:g2 { us:John us:knows us:Jack }" + "}";

        String q = "select * (xt:toList(xt:graph()) as ?list) " + "(let ((?e1) = ?list) {?e1} as ?t1)"
                + "(let ((?e1, ?e2) = ?list) {?e2} as ?t2)" + "(let ((?s, ?p, ?o) = ?t1) {?o} as ?v1)"
                + "(let ((?s, ?p, ?o) = ?t2) {?o} as ?v2)" + "where {" + "}";
        exec.query(i);
        Mappings map = exec.query(q);
    ...
    public void testedge() throws LoadException, EngineException {
        ...
        String q = "select * " + "where {" + "values ?t { unnest(xt:edges(us:age)) }" + "}"
    ...
    public void testvalue() throws LoadException, EngineException {
        ...
        String i = "insert data { " + "graph us:g1 { us:John us:age 10 } " + "graph us:g2 { us:John us:knows us:Jack }"
                + "}";

        String q = "select * (xt:index(?x) as ?i) (xt:index(?a) as ?j) (xt:index(us:age) as ?k) " + "where {"
                + "?x us:age ?a " + "filter (?a = xt:value(?x, us:age))" + "filter (?a = xt:value(?x, us:age, 1))"
    ...
    public void testLList() throws LoadException, EngineException {
        ...
        String q = "select " + "(xt:reverse(xt:iota(5)) as ?list)" + "(xt:member(xt:iota(5), 5) as ?member)"
                + "(xt:sort(xt:reverse(xt:iota(5))) as ?sort)" + "(xt:remove(xt:iota(5), 5) as ?remove) "
                + "(xt:removeindex(xt:iota(5), 0) as ?remindex) " + "(xt:swap(xt:iota(5), 0, 1) as ?swap)" + "where {"
                + "}" + "";
    ...
    public void testList() throws EngineException {
        QueryProcess exec = QueryProcess.create(Graph.create());
        String q = "select " + "(xt:list() as ?nil)" + "(xt:list(1, 2) as ?list)" + "(xt:cons(0, ?list) as ?res)"
                + "(xt:first(?res) as ?fst)" + "(xt:rest(?res) as ?rst)" + "(us:copy(xt:list(1, 2)) as ?cp)"
                + "(us:append(xt:list(1, 2), xt:list(3, 4)) as ?app)"
                + "(xt:list(1, 3) as ?ll) (xt:set(?ll, 1, 2) as ?set)" + "where {}"
                + "function us:copy(?list) { maplist(xt:self, ?list) }" + "" + "function us:append(?l1, ?l2) {"
                + "if (xt:size(?l1) = 0, us:copy(?l2)," + "xt:cons(xt:first(?l1), us:append(xt:rest(?l1), ?l2)))}" + "";
    ...
    public void testIterate() throws LoadException, EngineException {
        ...
        String q = "select (us:test() as ?s) " + "where {" + "}" + "" + "function us:test() {"
                + "let (?list = xt:list(), ?sum = 0){" + "for (?i in xt:iterate(1, 10)) {" + "set(?sum = ?sum + ?i)"
                + "};" + "?sum" + "}" + "}";
    ...
    public void testGName() throws LoadException, EngineException {
        ...
        String q = "select (st:cget(st:test1, st:value) as ?b1)  (st:cget(st:test2, st:value) as ?b2) " + "where {"
                + "bind (st:cset(st:test1, st:value, 10) as ?v1)" + "bind (st:cset(st:test2, st:value, 20) as ?v2)"
                + "}";
    ...
    public void testAccess() throws LoadException, EngineException {
        ...
        String q = "select " + "(xt:subject(?t) as ?s) (xt:property(?t) as ?p) "
                + "(xt:object(?t) as ?o)  (xt:graph(?t) as ?g) " + "where {" + "values ?t { unnest( xt:graph() )}"
                + "}";
    ...
    public void testPPath() throws LoadException, EngineException {
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String i = "insert data { " + "graph us:g1 { us:John us:name 'John'   ; us:knows us:James }"
    ...
    public void testSparql2() throws LoadException, EngineException {
        ...
        String q = "select (xt:sparql('select * (sum(?i) as ?sum) "
                + "where { values ?n {UNDEF} values ?i { unnest(xt:iota(?n))} }', '?n', 5) as ?res) " + "where {}";
    ...
    public void testNumber() throws LoadException, EngineException {
        Graph g = Graph.create();
        String q = "select (kg:number() as ?n) (sum(?n) as ?sum) where {" + "values ?e { unnest (xt:iota(5)) }" + "}";
    ...
    public void testJSON() throws EngineException, LoadException {
        String t = "template  {  st:apply-templates-with(st:json)}" + "where {}";
    ...
    public void testNG2() throws EngineException {
        ...
        String q = "select * " + "where {" + "bind (us:gg() as ?gg) " + "graph ?gg {" + "?x foaf:knows ?y "
                + "values ?g { us:g1 }" + " " + "{select ?y ?l  where {  graph ?g {  ?y rdfs:label ?l }}}" + "}" + "}"

                + "function us:gg() {" + "let (?g = construct {" + "graph us:g1 {" + "<John> foaf:knows <Jim>, <Jack> "
                + "} " + "graph us:g2 {" + "<Jim> rdfs:label 'Jim' " + "} } where {}) {" + "?g" + "}" + "}";
    ...
    public void testNG1() throws EngineException {
        ...
        String init = "insert data {" + "graph us:g1 {" + "<John> foaf:knows <Jim>, <Jack> " + "} " + "graph us:g2 {"
                + "<Jim> rdfs:label 'Jim' " + "}" + "}";

        String q = "select * " + "where {" + "?x foaf:knows ?y " + "values ?g { us:g1 }" + " "
    ...
    public void testfocus() throws EngineException, LoadException {
        ...
        String q = "select (us:foo() as ?f) where {" + "" + "}"
                ...
                + "function us:foo() {" + "let (?g = construct {us:John rdfs:label 'John'} where {}) {"
                + "xt:focus(?g, us:bar())" + "}" + "}"

                + "function us:bar() {" + "let (select (count(*) as ?c) where {?x ?p ?y}) { ?c }" + "}";
        Mappings map = exec.query(q);
    ...
    public void testbnode() throws EngineException, LoadException {
        ...
        String q = "select * where {" + "values ?n { 1 2 }" + "bind(bnode('a') as ?b1)" + "bind(bnode('a') as ?b2)"
                + "bind(bnode() as ?b)" + "}";
    ...
    public void testBindIndex7() throws EngineException, LoadException {
        ...
        String q = "select " + "(us:test(3, 3) as ?t) where {}"

                + "function us:test(?n, ?n)  {xt:display('here') ;" + "let (?n = ?n) {" + "for (?x in (xt:iota(?n))) {"
                + "if (?x > 1) {" + "return (?x)}" + "} " + "}" + "}";
    ...
    public void testBindIndex6() throws EngineException, LoadException {
        ...
        String q = "select " + "(us:test(3) as ?t) " + "where {}"
                ...
                + "function us:test(?n)  {" + "let (?n = ?z) {" + "for (?x in (xt:iota(?n))) {" + "if (?x > 1) {"
                + "return (?x)}" + "} " + "}" + "}";
    ...
    public void testBindIndex5() throws EngineException, LoadException {
        ...
        String q = "select (us:test(3) as ?t) where {}"
                ...
                + "function us:test(?n)  {" + "let (?n = ?n) {" + "for (?x in (xt:iota(?n))) {" + "if (?x > 1) {"
                + "return (?x)}" + "}" + "}" + "}";
    ...
    public void testBindIndex4() throws EngineException, LoadException {
        ...
        String q = "select " + "(let(?x = false, ?y = true) { ?y } as ?t1)"
                + "(if (?t1, let(?x = 2, ?y = 3) { ?y }, let(?z = 4) { ?z }) as ?t2)" + " where {}";
    ...
    public void testBindIndex3() throws EngineException, LoadException {
        ...
        String q = "select (us:test(2) as ?t) where {}"
                ...
                + "function us:test(?x) {" + "let ((?y, ?x) = @(1)) {" + "bound(?x);"
    ...
    public void testBindIndex() throws EngineException, LoadException {
        ...
        String q = "select (us:test(2) as ?t) where {}"
                ...
                + "function us:test(?x) {" + "let (?x = ?x * ?x) {" + "?x;" + "let (?x = ?x * ?x) {" + "?x}" + "}"
                + "}";
    ...
    public void testrecfun() throws EngineException {
        String q = "select (us:fac(5) as ?t) where {}" + "function us:fac(?n) {" + "if (?n = 0) {" + " return(1) " + "}"
                + "else {" + " let (?m = ?n, ?res = ?n * us:fac(?n - 1)) { " + "if (?m = ?n, ?res, 0)" + "}" + "}"
                + "}";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        Mappings map = exec.query(q);
    ...
    public void testExtFun21() throws EngineException {
        String init = "prefix ex: <http://example.org/> " + "insert data {" + "ex:John rdf:value 1 . "
                + "[] rdfs:label 2 ." + "us:Jim rdf:value (1 (2 3) 4 ())" + "us:James rdf:value ()" + "}";

        String q = "select  (us:foo(us:bar()) as ?t1) (us:foo(xt:graph()) as ?t2) (us:gee() as ?t3)" + "where {}"

                + "function us:foo(?g) {" + "query(construct  where { ?x ?p ?y }, ?g)" + "}"

                + "function us:bar() {" + "query(construct where {?x rdf:value ?y})" + "}"

                + "function us:gee() {" + "query(select * where { ?x ?p ?y}, query (construct where {?x rdf:value ?y}))"
                + "}";
    ...
    public void testExtFun20() throws EngineException {
        ...
        String init = "prefix ex: <http://example.org/> " + "insert data {" + "ex:John rdf:value 1 ; rdfs:label 2"
                + "}";
        ...
        String q = "select (datatype(query(construct where {?x ?p ?y})) as ?g) "
                ...
                + "(datatype( let((?b) = select * where {?x ?p ?y}) {?b}) as ?m)" + "(datatype(xt:iota(5)) as ?l)"
                + "where {}";
    ...
    public void testExtFun16() throws EngineException {
        ...
        String q = " select (us:foo() as ?t) " + "(us:bar() as ?tt) " + "where {}"

                + "function us:foo() {" + "maplist(" + "lambda((?x, ?p, ?y)) { xt:list(?x, ?p, ?y) }, "
                + "query(select * where {?x ?p ?y}))" + "}"

                + "function us:bar() {" + "maplist(" + "lambda((?s, ?p, ?o)) { xt:list(?s, ?p, ?o) }, "
    ...
    public void testExtFun15() throws EngineException {
        String q = "select (us:foo() as ?t) where {}" + "function us:foo() {"
    ...
    public void testExtFun14() throws EngineException {
        ...
        String q = "base <http://example.org/>" + "select (us:bar2() as ?t) (datatype(?t) as ?d) where {}"

                + "@public function us:bar2() {"
                + "let (?f1 = rq:uri, ?f2 = rq:replace, ?f3 = rq:sha256, ?f4 = xsd:integer) {" + "xt:list("
                + "xt:list(funcall(?f1, 'abc'), uri('abc')),"
                + "xt:list(funcall(?f2, 'abc', 'c', 'd'), replace('abc', 'c', 'd')), "
                // + "xt:list(funcall(?f3, 'abc'), sha256('abc')),"
                + "xt:list(sha256('abc'), sha256('abc'))," + "xt:list(funcall(?f4, '12'), xsd:integer('12'))" + ") "
                + "}" + "}";

        String q2 = "base <http://example2.org/>" + "select (us:bar2() as ?t)  where {}";
    ...
    public void testExtFun13() throws EngineException {
        ...
        String q = " select (us:bar() as ?t) (datatype(?t) as ?d) where {}"
                ...
                + "@public function us:bar() {" + "let (?fun = rq:regex) {"
    ...
    public void testExtFun11() throws EngineException {
        String init = "prefix ex: <http://example.org/> " + "insert data {" + "ex:John rdf:value 1, 2 ; rdfs:label 2 "
                + "}";

        String q = "prefix ex: <http://example.org/> " + "select * "
                + "(sum(let (?a = ?y + 1, ?b = exists { ?z ?p ?a }){ if (?b, 1, 0) }) as ?sum)" + "where {"
                + "?x ?p ?y "

                + "}"

                + "function us:foo(?x) {" + "exists { ?x ?p ?y }" + "}";
    ...
    public void testapply6() throws EngineException {
        ...
        String q = " " + "select (us:foo (5) as ?t) (us:foo (10) as ?r)" + "where {" + "}"

                + "function us:foo(?n){"
                + "let (?fun = if (?n <= 5, lambda(?x) { ?x * ?x }, lambda(?x) { ?x + ?x } )) {" + "us:bar(?fun, ?n)"
                + "}" + "}"

                + "function us:bar(?fun, ?n) {" + "funcall(?fun, ?n)" + "}";
    ...
    public void testapply5() throws EngineException {
        ...
        String q = "select " + "(funcall (let (?fun = lambda(?x) { ?x * ?x }) { ?fun }, 5) as ?t) " + "where {" + "}";
    ...
    public void testapply44() throws EngineException {
        ...
        String q = "select * where {" + "select (us:foo() as ?t) where {" + "}" + "}"

                + "function us:foo(){" + "let (select * where {select " + "(maplist(lambda(?x) {  "
                + "exists { select * where { select * where { values ?x { UNDEF } filter exists { select * where {select * where { ?y ?p ?x }}}}}}"
                + "}, xt:iota(5)) as ?t) " + "where {}}) {" + "?t }" + "}";
    ...
    public void testapply4() throws EngineException {
        ...
        String q = "select * where {" + "select (us:foo() as ?t) where {" + "}" + "}"
                ...
                + "function us:foo(){" + "let (select * where {select "
                // + "(maplist(lambda(?x) { exists { ?y ?p ?x }}, xt:iota(5)) as ?t) "
                + "(maplist(lambda(?x) {  exists { select * where { ?y ?p ?x }}}, xt:iota(5)) as ?t) " + "where {}}) {"
                + "?t }" + "}";
    ...
    public void testapply22() throws EngineException {
        QueryProcess exec = QueryProcess.create(Graph.create());
        String q = "select * where {" + "values ?t { unnest(us:test()) }" + "}"

                + "function us:test(){" + "let( " + "select * where {"
                + "select  * (reduce(rq:plus,   xt:iota(5)) as ?sum1) "
                + "(reduce(rq:concat, xt:iota('a', 'c')) as ?con)" + "(reduce(rq:mult,   xt:iota(5)) as ?mul)"
                + "where {" + "bind  (reduce(rq:plus,   xt:iota(5)) as ?sum2) " + "filter (?sum2 > 1) "
                + "filter exists { "
    ...
    public void testLetQuery2() throws EngineException {
        String init = "insert data {" + "us:John us:child us:Jim, us:Jane, us:Janis ." + "us:Jane a us:Woman ;"
                + "us:child us:Jack, us:Mary ." + "us:Mary a us:Woman ." + "us:Janis a us:Woman ;"
                + "us:child us:James, us:Sylvia ." + "us:Sylvia a us:Woman ." + "}";

        String q = "select * where {" + "values ?e { unnest(us:pattern()) }" + "}"

                + "function us:pattern(){" + "let (?sol = select * where { ?x us:child ?y optional { ?y a ?t } } ){"
                + "  maplist(us:collect, ?sol)     " + "}" + "}"

                + "function us:collect(?map){" + "let ((?y, ?t) = ?map){" + "xt:list(?y, coalesce(?t, us:Person))" + "}"
                + "}";
    ...
    public void testLetQuery() throws EngineException {
        ...
        String q = "select *  where {" + "values ?t { unnest(us:pattern(us:John)) } " + "}"

                + "function us:pattern(?x){"
                + "    let (select ?x (xt:cons(aggregate(?y), aggregate(us:pattern(?y))) as ?l) "
                + "         where { ?x us:child ?y . ?y a us:Woman }" + "         group by ?x){"
                + "        reduce(xt:merge, ?l)" + "    }" + "}";
    ...
    public void testDatatypeValue10() throws EngineException {
        String init = "insert data {" + "graph us:g1 { <Jack> rdf:value  1,  1.0 }"

                + "graph us:g2 { <Jack> rdf:value  1,  1.0 }" + "} ";

        String q = "select * (kg:index(?v) as ?i) where {" + "?x ?p ?v " + "?y ?p ?v " + "}";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
        Mappings map = exec.query(q);
    ...
    public void testDatatypeValue9() throws EngineException {
        String init = "insert data {" + "graph us:g1 { <Jack> " + "rdf:value  "
        ...
        String q = "select * (kg:index(?v1) as ?i) where {" + "graph ?g1 {?x ?p ?v1} " + "graph ?g2 {?y ?p ?v2} "
                + "filter sameTerm(?v1, ?v2)" + "filter (?g1 < ?g2)" + "}";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
        Mappings map = exec.query(q);
    ...
    public void testDatatypeValue8() throws EngineException {
        ...
        String q = "select * (kg:index(?v) as ?i) where {" + "graph ?g1 {?x ?p ?v} " + "graph ?g2 {?y ?p ?v} "
                + "filter (?g1 < ?g2)" + "}";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
    ...
    public void testDatatypeValue7() throws EngineException {
        String init = "insert data {" + "graph us:g1 { <Jack> " + "rdf:value  "
                + " 1,  1.0,  '1'^^xsd:long, 1e0, '1'^^xsd:double, '1'^^xsd:float, "
                + "true, '1'^^xsd:boolean, false,   01, " + "'1'^^xsd:boolean, '0'^^xsd:boolean . }"

                + "graph us:g2 { <Jack> rdf:value false, '01'^^xsd:double, '1.0'^^xsd:float,"
                + "'01'^^xsd:integer, '1'^^xsd:integer"
                + ", '1'^^xsd:short, '1'^^xsd:byte , '1'^^xsd:int, '01'^^xsd:byte, '1'^^xsd:byte}" + "} ";

        String q = "select *  where {" + "?x ?p ?v" + "}" + "order by ?x ?v ";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
        Mappings map = exec.query(q);
    ...
    public void testDatatypeValue6() throws EngineException {
        ...
        String q = "select *  where {" + "graph ?g {" + "?x ?p ?v" + "} " + "}" + "order by ?x ?v ";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
        Mappings map = exec.query(q);
    ...
    public void testDatatypeValue5() throws EngineException {
        String init = "insert data {" + "<Jack> " + "rdf:value  "
                + " 1,  1.0,  '1'^^xsd:long, 1e0, '1'^^xsd:double, '1'^^xsd:float, "
                + "true, '1'^^xsd:boolean, false,   01, " + "'1'^^xsd:boolean, '0'^^xsd:boolean ."

                + " <Jim> rdf:value false, '01'^^xsd:double, '1.0'^^xsd:float," + "'01'^^xsd:integer, '1'^^xsd:integer"
                + ", '1'^^xsd:short, '1'^^xsd:byte , '1'^^xsd:int, '01'^^xsd:byte, '1'^^xsd:byte" + "} ";

        String q = "select *  where {" + "?x ?p ?v filter (?v = 1)" + "}" + "order by ?x ?v ";
    ...
    public void testDatatypeValue4() throws EngineException {
        ...
        String q = "select *  where {" + "?x ?p ?v " + "}" + "order by ?x ?v " + "values ?v { 1 }";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
        Mappings map = exec.query(q);
    ...
    public void testDatatypeValue3() throws EngineException {
        ...
        String q = "select *  where {" + "?x ?p 1" + "}" + "order by ?x";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
        Mappings map = exec.query(q);
    ...
    public void testDatatypeValue2() throws EngineException {
        ...
        String q = "select *  where {" + "?x ?p ?v " + "?y ?p ?v " + "filter (?x != ?y)" + "}";
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        exec.query(init);
        Mappings map = exec.query(q);
    ...
    public void testDatatype1() throws EngineException {
        String init = "insert data {" + "<John> foaf:age '21'^^xsd:double, 21 " + "<Jack> foaf:age 21e0, 21.0 " + "} ";

        String query = "select distinct ?a where {" + "?x foaf:age ?a " + "?y foaf:age ?a " + "filter (?x != ?y)"
    ...
    public void testDatatype2() throws EngineException {
        String init = "insert data {" + "<John> rdf:value true " + "<Jack> rdf:value '1'^^xsd:boolean " + "} ";

        String query = "select distinct ?a where {" + "?x ?p ?a " + "?y ?p ?a " + "filter (?x != ?y)"
    ...
    public void testGenAggOld() throws EngineException {
        String q = "select (st:aggregate(?x) as ?y) " + "where { values ?x {unnest(xt:iota(10))}}"
    ...
    public void testReturn() throws EngineException {
        ...
        String q = "select (us:fun(5) as ?n) where {}" + "function us:fun(?n) {" + "for (?x in xt:iota(?n)){"
                + "if (?x = ?n){" + "return (?x)}" + "}" + "}";
    ...
    public void testOptionalFilter() throws LoadException, EngineException {
        ...
        String i = "insert data {" + "us:John foaf:knows us:James " + "us:James foaf:knows us:Jack, us:Jim "
                + "us:Jim foaf:knows us:John " + "" + "}";

        String q = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  *  " + "" + "where { "
                + " ?x foaf:knows ?y " + "optional { " + "?y foaf:knows ?z filter (?x != us:John) optional { ?z ?p ?x }"
                + "}  " + "} ";
    ...
    public void testDataStore() throws EngineException {
        ...
        String init = "insert data {" + "graph us:g1 {" + "us:John foaf:knows us:Jack, us:Jules "
                + "foaf:knows rdfs:domain foaf:Person" + "} " + "graph us:g2 {" + "us:John foaf:knows us:Jack "
                + "foaf:Person rdfs:subClassOf foaf:Human " + "foaf:Human rdfs:subClassOf foaf:Humanoid " + "}" + "}";
        ...
        assertEquals(4, count(g.getNamed().iterate().filter(df.init().or().and().graph(ExprType.EQ, g1)
                .subject(ExprType.EQ, n).and().graph(ExprType.EQ, g2).not().subject(ExprType.EQ, n))));
        ...
        assertEquals(0, count(
                g.getDefault().iterate().filter(df.init().edge(ExprType.EQ, DataFilter.SUBJECT, DataFilter.OBJECT))));
    }
    ...
    public void testTTLabc() throws EngineException, LoadException {
        ...
        String i = "insert data {  " + "_:b rdf:value _:b, (_:b (_:b) ((_:b)))  " + "}";
    ...
    public void testIndex() throws EngineException {
        String q = "select * where {" + "bind (1 as ?x) " + "values (?x ?y) {" + "(2 2) (1 1)" + "}" + "}";
    ...
    public void testAGG22() throws EngineException {
        String q = "select * (max(?c) as ?mc) where {" + "select (count(*) as ?c) (max(?y) as ?m) where {?x ?p ?y}"
                + "}";
    ...
    public void testContext2() throws EngineException {
        ...
        QueryProcess exec = QueryProcess.create(g);
        String q = "template { st:get(st:test) }" + " where {" + "select * where {" + "bind (us:test() as ?t)" + "}"
                + "}" + "function us:test(){" + "st:set(st:test, 10)" + "}";
    ...
    public void testContext() throws EngineException {
        Graph g = GraphStore.create();
        QueryProcess exec = QueryProcess.create(g);
        String q = "select * (st:get(st:test) as ?tt) where {" + "select * where {" + "bind (us:test() as ?t)" + "}"
                + "}" + "function us:test(){" + "st:set(st:test, 10)" + "}";
    ...
    public void testGLoop() throws EngineException, IOException {
        ...
        QueryProcess exec = QueryProcess.create(g);
        String q = "select (us:test() as ?t)" + "where {}" + "" + "function us:test(){"
                + "loop ((?s, ?p, ?o) in construct {us:John rdfs:label 'Jon', 'Jim' } where {}){" + "?o" + "}" + "}";
    ...
    public void testUnion5() throws EngineException {
        ...
        String i1 = "insert data {" + "us:John foaf:knows us:Jim " + "us:Jim foaf:knows us:Jack " + "}";

        String q = "select (us:main() as ?m) where {}"

                + "function us:main(){" + "let (?list = xt:list(us:test1, us:test2)){"
                + "reduce(xt:union, maplist(rq:funcall, ?list))" + "}" + "}"

                + "function us:test1(){" + "let (?m = select *  where { ?x foaf:knows ?y}){" + "?m}" + "}"

                + "function us:test2(){" + "let (?m = select *  where { ?z foaf:knows ?t}){" + "?m}" + "}";
    ...
    public void testUnnestSelectCons() throws EngineException {
        ...
        String q = "prefix ex: <htp://example.org/>" + "select *  where {" + "values (?x ?y ?path) {unnest(us:test())}"
                + "graph ?path { ?a ?p ?b }" + "}"

                + "function us:test(){" + "let (?m = select * where {" + "bind (us:graph() as ?g)"
                + "graph ?g {?x foaf:knows+ :: ?path  ?y}" + "})" + "{?m}" + "}"

                + "function us:graph(){" + "let (?g = construct  {" + "us:John foaf:knows us:Jack "
                + "us:Jack foaf:knows us:Jim} where {})" + "{?g}" + "}";
    ...
    public void testUnnestGraph() throws EngineException {
        ...
        String q = "prefix ex: <htp://example.org/>" + "select ?s ?p ?o  where {" + "bind (us:test() as ?g)"
                + "values (?s ?p ?o) {unnest(?g)}" + "graph ?g {?s ?p ?o}" + "}" + "function us:test(){"
                + "let (?g = construct { us:John rdfs:label 'John', 'Johnny' } where {})" + "{?g}" + "}";
    ...
    public void testUnnestSelect() throws EngineException {
        ...
        String q = "prefix ex: <htp://example.org/>" + "select *  where {" + "values (?x ?y) {unnest(us:test())}" + "}"
                + "function us:test(){" + "let (?m = select * where { " + "values ?x {unnest(xt:iota(2))}  "
                + "values ?z {unnest(xt:iota(1))}  " + "values ?y {unnest(xt:iota(2))}})" + "{?m}" + "}";
    ...
    public void testUnnestSelect2() throws EngineException {
        ...
        String q = "prefix ex: <htp://example.org/>" + "select *  where {" + "values (?y ?x) {unnest(us:test())}" + "}"
                + "function us:test(){" + "let (?m = select * where { " + "values ?x {unnest(xt:iota(2))}  "
                + "values ?z {unnest(xt:iota(1))}  " + "values ?y {unnest(xt:iota(2))}})" + "{?m}" + "}";
        Mappings map = exec.query(q);
        assertEquals(4, map.size());
    ...
    public void testFunUpdate() throws EngineException {
        Graph g = GraphStore.create();
        QueryProcess exec = QueryProcess.create(g);
        String i = "insert { us:John foaf:name ?n } " + "where { bind (us:name(us:John) as ?n) }"
    ...
    public void testNSMUnnest() throws EngineException {
        ...
        String i = "prefix ex: <htp://example.org/>" + "insert data { " + "us:John rdfs:label 'John'"
                + "ex:John rdfs:label 'John'" + "}";

        String q = "prefix ex: <htp://example.org/>" + "select * " + "where {" + "?x ?p ?y "
                + "filter mapany(us:test, ?x, st:prefix())" + "}" + "function us:test(?uri, ?def){"
                + "let ((?p, ?n) = ?def){" + "strstarts(?uri, ?n)" + "}" + "}";
    ...
    public void testQueryUnnest() throws EngineException {
        ...
        String q = "prefix ex: <htp://example.org/>" + "select * " + "where {" + "?x ?p ?y optional {?x ?p ?p }"
                + "values ?t { unnest(xt:query()) }" + "values ?e { unnest(?t) }" + "}";
    ...
    public void testGraphUnnest() throws EngineException {
        ...
        String q = "select * where {" + "values (?s ?p ?o) { unnest(us:define()) }"
                + "values (?s ?p ?o ?g) { unnest(us:define()) }" + "?s ?p ?o " + "}" + "function us:define(){"
    ...
    public void testUnnestNSM() throws EngineException {
        ...
        String q = "prefix ex: <htp://example.org/>" + "select *  where {" + "values (?p ?n) {unnest(st:prefix())}"
                + "}" + "function us:test(){" + "for ((?p, ?n) in st:prefix()){" + "xt:display(?p, ?n)" + "}" + "}";
    ...
    public void testExtNode() throws EngineException {
        ...
        String q = "select * where {" + "bind (us:define() as ?g)" + "?p rdfs:label ?l " + "graph ?g {"
                + "?p rdfs:label ?ll" + "}" + "}" + "function us:define(){"
                + "let (?g = construct {us:prop1 rdfs:label 'prop' us:prop rdfs:label 'prop2'} where {})" + "{ ?g }"
                + "}";
    ...
    public void testUnion4() throws EngineException {
        ...
        String q = "function xt:main(){" + "us:test()" + "}" + "function us:test(){" + "let ("
                + "?m1 = select *  where { ?x foaf:knows ?y}," + "?m2 = select *  where { ?x foaf:knows ?y}){"
                + "xt:union(?m1, ?m2)" + "}" + "}";
    ...
    public void testUnion2() throws EngineException {
        ...
        String q = "function xt:main(){" + "us:test()" + "}" + "function us:test(){" + "let (?g = us:union(),"
                + "?m = select * where { graph ?g {?x foaf:knows+ ?y}})" + "{ ?m }" + "}" + "function us:union(){"
                + "let (" + "?g1 = construct {us:John foaf:knows us:Jim}  where {},"
                + "?g2 = construct {us:Jim  foaf:knows us:Jack} where {}){" + "xt:union(?g1, ?g2)" + "}" + "}";
    ...
    public void testUnion() throws EngineException {
        ...
        String i1 = "insert data {" + "us:John foaf:knows us:Jim " + "}";

        String i2 = "insert data {" + "us:Jim foaf:knows us:Jack " + "}";
    ...
    public void testRelax6() throws EngineException {
        ...
        String i = "insert data { " + "us:John foaf:name 'John'@en ; foaf:age 11 " + "}";
        exec.query(i);

        String q = "@relax " + "select * (xt:sim() as ?s) where {" + "us:John foaf:name 'Jon' ; foaf:age 11 " + "}";
    ...
    public void testRelax5() throws EngineException {
        ...
        String i = "insert data { " + "us:John foaf:name 'John' " + "}";
        exec.query(i);

        String q = "@relax kg:uri " + "select * where {" + "us:Jim foaf:name 'John' " + "}";
    ...
    public void testRelax4() throws EngineException {
        ...
        String q = "@relax * " + "select * where {" + "?x rdfs:label 'John' " + "}";
    ...
    public void testRelax3() throws EngineException {
        ...
        String q = "@relax kg:property " + "select * where {" + "?x rdfs:label 'John' " + "}";
    ...
    public void testRelax2() throws EngineException {
        ...
        String q = "@relax kg:literal " + "select * where {" + "?x rdfs:label 'Jon' " + "}";
    ...
    public void testRelax1() throws EngineException {
        ...
        String q = "@relax " + "select * where {" + "?x rdfs:label 'Jon' " + "}";
    ...
    public void testApprox() throws EngineException, LoadException {
        ...
        String i = "insert data {" + "us:John us:name 'John' " + "us:Jack us:name 'Jack' " + "us:Jim  us:name 'Jim' "
                + "}";

        String q = "@relax * " + "select * (xt:sim() as ?s) where {" + "?x xt:name 'Jon'" + "}" + "order by desc(?s)";
    ...
    public void testSubqueryFun() throws EngineException {
        ...
        String i = "insert data { us:John rdfs:label 'John' }" + "@public  {" + "function us:test(){"
                + "let ((?m) = select * where {?x ?p ?y}, (?y)=?m" + "){?y}" + "}" + "}";

        String q = "select * (us:test() as ?r) (us:foo() as ?s) " + "where {"
                + "select * where {select * (us:test() as ?t) (us:foo() as ?f) " + "where {}}" + "}"
                + "function us:foo(){" + "let ((?m) = select * where {?a ?q ?b}, (?b)=?m){ us:test() }" + "}";
    ...
    public void testImport() throws EngineException {
        ...
        String q = "prefix ex: <http://ns.inria.fr/sparql-extension/aggregate#>" + "select * "
                + "(funcall(ex:median, xt:iota(5)) as ?m)" + "(ex:sigma(xt:iota(5)) as ?s)" + "" + "where {}";
    ...
    public void testCustom() throws EngineException {
        ...
        String q = "function xt:main(){" + "cs:test(10) " + "}";
    ...
    public void testAnnot() throws EngineException {
        ...
        String i = "@public {" + "function us:foo(){" + "us:bar()" + "}" + " function us:bar(){" + "10" + "}" + "}";
    ...
    public void testSPARQLfun() throws EngineException {
        ...
        String q = "select " + "(rq:isURI(us:test) as ?uri)" + "(rq:isBlank(us:test) as ?bn)"
                + "(mapany(rq:strstarts, us:test, xt:list(xt:, st:, us:)) as ?st)" + "where {}";
        Mappings map = exec.query(q);
    ...
    public void testUnbound() throws EngineException {
        ...
        String q = "select (us:test() as ?res)" + "where {}" + "function us:test(){"
    ...
    public void testLetService() throws EngineException {
        ...
        String q = "prefix r: <http://fr.dbpedia.org/resource/>" + "select (us:dbpedia(?x, rdfs:label) as ?t)"
                + "where {" + "}" + "values ?x {r:Auguste}" + "function us:dbpedia(?x, ?p) {" + "  let((?l) ="
                + "    service <http://fr.dbpedia.org/sparql> {" + "	?x ?p ?l" + "      })" + "  {?l}" + "}";
    ...
    public void testFuncall() throws EngineException {
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        String q = "function xt:main(){" + "let (?fun = us:test){" + "funcall(?fun, 'Hello')}" + "}"
                + "function us:test(?m){" + "?m" + "}";
    ...
    public void testMethod() throws EngineException {
        ...
        String init = "insert data {" + "foaf:Man rdfs:subClassOf foaf:Person " + "us:John a foaf:Man "
                + "us:Boat a foaf:Thing " + "us:bar a xt:Method ;" + "xt:name us:title ;" + "xt:input (foaf:Person) ;"
                + "xt:output xsd:string ." + "" + "us:foo a xt:Method ;" + "xt:name us:title ;"
                + "xt:input (foaf:Thing) ;" + "xt:output xsd:string ." + "}";

        String q = "select (funcall(us:method(us:title, ?x), ?x) as ?t) where {" + "?x a foaf:Man" + "}"

                + "function us:method(?m, ?x){" + "let ((?res) = select * where {"
                + "?x rdf:type/rdfs:subClassOf* ?t . " + "?fun a xt:Method ; xt:name ?m ; xt:input(?t)},  (?fun)=?res )"
                + "{ ?fun }" + "}"

                + "function us:bar(?x){" + "'bar'" + "}"

                + "function us:foo(?x){" + "'foo'" + "}";
    ...
    public void testMain() throws EngineException {
        ...
        String i = "insert data {" + "[] a us:Test ; us:width 2 ; us:length 3 " + "}";

        String q = "function xt:main(){" + "us:test()" + "}" + "function us:test(){"
                + "let (select ?x (us:surface(?x) as ?s) where {?x a ?t} ) {"
                // + "kg:display(?x); kg:display(?s);"
                + "?s" + "}}" + "function us:surface(?x){" + "let (select * where {?x us:length ?l ; us:width ?w }){"
                + "?l * ?w}" + "}";
    ...
    public void testexistsexport() throws EngineException {
        ...
        String init = "insert data {" + "[] rdf:value 1" + "}";

        String qe = "select where {}" + "@public {" + "function us:test(){" + "if (exists {select * where {?x ?p ?y}}){"
                + "let ((?m) = select * where {?x ?p ?y}, (?x, ?y)=?m){" + "?y}" + "}" + "else {us:fun(false)}" + "}"
                + "function us:fun(?x){" + "?x}" + "}";
    ...
    public void testexists() throws EngineException {
        ...
        String i = "insert data {" + "[] us:width 2 ; us:length 3" + "[] us:width 3 ; us:length 4" + "}";

        String q = " select * (us:surface(?x) as ?s) where {" + "?x us:width ?w " + "}" + "function us:surface(?x){"
                + "let ((?m) = select * where { ?x us:length ?l ; us:width ?w }, (?l, ?w)=?m){" + "?l * ?w" + "}" + "}";

        String q2 = "select (us:test() as ?s) where {}" + "function us:test(){" + "let (?sum = 0){"
                + "for (?m in select * where { ?x us:width ?w }){" + "let ((?w, ?x) = ?m){  " + "set (?sum = ?sum + ?w)"
                + "}}; ?sum" + "}" + "}";
    ...
    public void testBnode() throws EngineException, LoadException, IOException {
        ...
        String i = "prefix ex: <http://example.org/>" + "insert data {"
                + "[] ex:speed [ rdf:value 100 ; ex:unit 'km/h' ]" + "[] ex:speed [ rdf:value 90 ; ex:unit 'km/h' ]"
                + "" + "" + "}";

        String q = "prefix bn: <http://ns.inria.fr/sparql-extension/bnode#>" + "prefix ex: <http://example.org/>"
                + "select  ?x ?z where {" + "?x ex:speed ?y " + "?z ex:speed ?t " + "filter (?x != ?z) "
                + "filter (?y <= ?t) " + "}" + "function bn:lessEqual(?x, ?y){"
                + "let (?v1 = xt:value(?x, rdf:value), ?v2 = xt:value(?y, rdf:value)){" + "?v1 <= ?v2}" + "} ";
    ...
    public void testSet() throws EngineException {
        ...
        String q = "select (us:test() as ?t) where {}" + "function us:test(){" + "let (?sum = 0){ "
                + "for (?x in xt:iota(100)){" + "if (xt:prime(?x)){" + "set(?sum = ?sum + 1)" + "}};" + "?sum" + "}"
                + "}";
    ...
    public void testCandidate() throws EngineException {
        ...
        String q = "select * where {" + "bind (unnest(xt:iota(3)) as ?n)" + "}" + "function xt:solution(?q, ?ms){"
                + "map (kg:display, ?ms)" + "}";
    ...
    public void testSolution() throws EngineException {
        ...
        String q = "select * where {" + "bind (unnest(xt:iota(100)) as ?n)" + "}" + "function xt:solution(?q, ?ms){"
                + "for (?m in ?ms){" + "if (! us:check(?m)){" + "xt:reject(?m)}" + "}" + "}" + "function us:check(?m){"
                + "rand() <= 0.5" + "}";
    ...
    public void testCustomAgg() throws EngineException {
        ...
        String init = "insert data {" + "[] rdf:value 4, 5, 6, 7" + "[] rdf:value 1, 2, 3" + "}";

        String q = "select (aggregate(?v) as ?list) ( us:mediane(?list) as ?res)" + "where {" + "  ?x rdf:value ?v "
                + "}" + "" + "function us:mediane(?list){" + "  let (?l = xt:sort(?list)){"
    ...
    public void testVD13() throws EngineException {
        ...
        String q = "prefix cal: <http://ns.inria.fr/sparql-extension/calendar/>" + "select ?day  where {"
    ...
    public void testMapList() throws EngineException {
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String q = "select * where {" + "bind (maplist (rq:plus, xt:iota(1, 10), xt:iota(1, 10)) as ?res)" + "}" + "";

        String q1 = "select * where {" + "bind (maplist (rq:plus, xt:iota(1, 10), 10) as ?res)" + "}" + "";

        String q2 = "select * where {" + "bind (maplist (rq:plus, 10, xt:iota(1, 10)) as ?res)" + "}" + "";

        String q3 = "select * where {" + "bind (maplist (rq:plus, xt:iota(1, 20), xt:iota(1, 10)) as ?res)" + "}" + "";
    ...
    public void testExtList() throws EngineException {
        ...
        String q = "select ?n  " + "where {" + "bind (unnest(xt:iota(1, 100)) as ?n)" + "filter  xt:prime(?n)" + "}";

        String q2 = "select * where {" + "bind (mapfindlist (xt:prime, xt:iota(100)) as ?test)" + "}";
    ...
    public void testExtFun10() throws EngineException {
        String init = "prefix ex: <http://example.org/> " + "insert data {" + "ex:John rdf:value 1, 2, 3 " + "}";

        String q = "prefix ex: <http://example.org/> " + "select  * " + "where {" + "?x rdf:value ?n "
                + "filter xt:test(?n)" + "filter xt:fun()" + "}" + "function xt:test(?n) { " + "if (?n = 1, "
                + "xt:test(?n + 1)," + "let (?m = ?n + 1){ exists { ?x rdf:value ?m }}" + ") " + "}"
    ...
    public void testExtFun9() throws EngineException {
        String init = "prefix ex: <http://example.org/> " + "insert data {" + "ex:John rdf:value 1, 2 " + "}";

        String q = "prefix ex: <http://example.org/> " + "select *" + "where {" + "?x rdf:value ?n "
                + "filter exists { ?y rdf:value ?n " + "filter (let (?z = 3){ exists { ?t ?p ?z}}) } " + "}";
    ...
    public void testExtFun8() throws EngineException {
        ...
        String q = "prefix ex: <http://example.org/> " + "select * " + "where {" + "?x rdf:value ?n "
                + "filter exists { select { ?y ?p ?n  filter ( xt:fun(?n)) } } " + "}" + "function xt:fun(?n) { "
                + "exists { ?x ?q ?n } " + "} ";
    ...
    public void testExtFun7() throws EngineException {
        ...
        String q = "prefix ex: <http://example.org/> " + "select (sum(xt:fun(?n + 1)) as ?sum)" + "where {"
                + "?x rdf:value ?n " + "}" + "function xt:fun(?n) { "
    ...
    public void testExtFun3() throws EngineException {
        ...
        String q = "prefix ex: <http://example.org/> " + "select * " + "where {" + "?x rdf:value ?n "
                + "filter exists { ?y ?p ?n filter (! xt:fun(?n)) } " + "}" + " function xt:fun(?n) { "
                + "exists { ?n ?q ?x } " + "} ";
    ...
    public void testExtFun1() throws EngineException {
        ...
        String q = "prefix ex: <http://example.org/> " + "select * " + "where {" + "?x rdf:value ?n "
                + "filter xt:fun(?n) " + "}" + "function xt:fun(?n) { " + "exists { ?y ?p ?n } " + "} ";
    ...
    public void testExtFun5() throws EngineException {
        ...
        String q = "prefix ex: <http://example.org/> " + "select (sum(xt:fun(?n + 1)) as ?sum)" + "where {"
                + "?x rdf:value ?n " + "}" + "function xt:fun(?n) { if (exists { ?x ?p ?n }, 1, 0) } ";
    ...
    public void testExtFun4() throws EngineException {
        ...
        String q = "prefix ex: <http://example.org/> " + "select (sum(xt:fun(?n)) as ?sum)" + "where {"
                + "?x rdf:value ?n " + "}" + "function xt:fun(?n) { if (exists { ?x ?p ?n }, 1, 0) } ";
    ...
    public void testExtAgg() throws EngineException {
        ...
        String q = "prefix ex: <http://example.org/> " + "select (sum(xt:fun(?n)) as ?sum)" + "where {"
                + "?x rdf:value ?n " + "}" + "function xt:fun(?x)  { ?x + ?x } ";
    ...
    public void testExistFunRec() throws EngineException, LoadException {
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        String init = "prefix ex: <http://example.org/>" + "insert data {" + "ex:a a ex:Case " + "ex:a ex:p ex:b "
                + "ex:b ex:q ex:c " + "ex:c ex:r ex:a " + "" + "ex:c a ex:Case " + "ex:c ex:p ex:d " + "ex:d ex:q ex:e "
                + "ex:e ex:r ex:c " + "ex:e a ex:Case " + "ex:e ex:p ex:f " + "ex:f ex:q ex:g " + "ex:g ex:r ex:e "
                + "}";

        String q = "prefix ex: <http://example.org/>" + "select *" + "where {" + "?x a ex:Case "
                + "filter xt:test(?x, 2, 0)" + "}" + "function xt:test(?x, ?n, ?m) {" + "if (?m >= ?n, true,"
                + "exists { ?x ex:p ?y . ?y ex:q ?z . ?z ex:r ?x " + "filter xt:test(?z, ?n, ?m + 1) }" + ") }";
    ...
    public void testGeneralize() throws EngineException, LoadException, IOException {
        ...
        QueryProcess exec = QueryProcess.create(g);
        String init = "prefix ex: <http://example.org/>" + "insert data { " + "ex:John a ex:OldMan ;"
                + "ex:author [ a ex:Document ]" + "ex:Jack a ex:Person ;" + "ex:author [ a ex:Document ]"
                + "ex:Man       rdfs:subClassOf ex:Human " + "ex:YoungMan  rdfs:subClassOf ex:Man "
                + "ex:OldMan    rdfs:subClassOf ex:Man " + "ex:Report    rdfs:subClassOf ex:Document  " + "}";

        // target type more general than query
        // target type brother of query
        String qq = "prefix ex: <http://example.org/>" + "select * (kg:similarity() as ?sim) " + "where {"
                + "?x a ex:YoungMan, ?tt ;" + "ex:author [ a ex:Report ] " + "" + "}"
                + "function xt:candidate(?q, ?t, ?b) { " + "let ((?qs, ?qp, ?qo) = ?q, "
                + "     (?ts, ?tp, ?to) = ?t) {" + "if (?qp = rdf:type && isURI(?qo), "
                + "?b || exists { ?qo rdfs:subClassOf/(rdfs:subClassOf*|^rdfs:subClassOf) ?to } ," + "?b)" + "}" + "}";
    ...
    public void testNoGlobal() throws EngineException {
        Graph g = Graph.create();
        QueryProcess exec = QueryProcess.create(g);
        String init = "insert data { " + "<John> rdf:value 1, 2  ." + "" + "}";

        String qq = "select *" + "where {" + "bind (xt:test(?x) as ?z)" + "?x ?p ?y " + "}"
    ...
    public void testFunExist() throws EngineException {
        ...
        String q = "select " + "(xt:test(?n) as ?r) " + "(xt:test(4) as ?b)" + "where {" + "?x rdf:value ?n "
                + "filter (xt:test(?n))}" + "function xt:test(?y) {  exists {?z rdf:value ?y} }";
    ...
    public void testFunfdghf() throws EngineException {
        String q = "select \n" + "(xt:f(1) as ?x)\n" + "(xt:f(1, 2) as ?y)\n" + "\n" + "where {}"
                + "function xt:f(?x) { ?x }" + "function xt:f(?x, ?y) { ?x + ?y }"
    ...
    public void testFuture() throws EngineException {
        ...
        String q = "template {" + "st:number() ' : ' ?y}" + "where {" + "?x rdf:value ?y " + "} order by desc(?y)";
    ...
    public void testCal2() throws EngineException, LoadException {
        ...
        String qq = "prefix cal: <http://ns.inria.fr/sparql-extension/calendar/>" + "select *" + "where {" + "?x ?p ?y "
                + "filter (cal:jour(?y) = 'Mardi' )" + "}";

        String init = "insert data { " + "<Day1> rdf:value '2015-06-16'^^xsd:date ."
                + "<Day2> rdf:value '2015-06-17'^^xsd:date ." + "<Day3> rdf:value '2015-06-23'^^xsd:date ." + "}";
        exec.query(init);
    ...
    public void testBBB() throws EngineException {
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        String init = "insert data { " + "<John> rdf:value 5 ." + "<Jim>  rdf:value 30}";
        String q = "select * " + "where {" + "?x rdf:value ?y " + "filter (xt:foo(?x, 10) || xt:foo(?x, 5))" + "} "
    ...
    public void testreduce() throws EngineException {
        QueryProcess exec = QueryProcess.create(Graph.create());
        String q = "select " + "(reduce(rq:concat, xt:iota('a', 'c')) as ?con)"
                + "(reduce(rq:plus,   xt:iota(5)) as ?sum)" + "(reduce(rq:mult,   xt:iota(5)) as ?mul)" + "where {}";
        Mappings map = exec.query(q);
    ...
    public void testEEE2() throws EngineException {
        ...
        String q = "select " + "(reduce  (rq:plus, maplist (xt:fun, xt:iota(0, 12))) as ?res)" + "where {}"
    ...
    public void testEEE() throws EngineException {
        ...
        QueryProcess exec = QueryProcess.create(g);
        String init = "insert data { <John> rdf:value 1, 2, 3, 4, 5, 6, 7, 8 .}";
        String q = "select * " + "(1 + sum(xt:foo(xsd:long(?n))) as ?res)" + "where {" + "?x rdf:value ?n" + "}"
                + "function xt:fac(?n) { if (?n = 0, 1, ?n * xt:fac(?n - 1)) }"
    ...
    public void testExtFun() throws EngineException, LoadException {
        ...
        String query = "select " + "( st:test(st:fac(?x)) as ?r)" + "where {" + "bind (5 as ?x)" + "}"
                + "function st:fac(?x)  { if (?x = 1, 1, ?x * st:fac(?x - 1)) }"
                + "function st:test(?x) { let(?y = ?x * ?x){ ?y} }";

        String query2 = "select " + "(st:test(st:fac(?x)) as ?r)" + "where {" + "bind (5 as ?x)}"
                + "function st:fac(?x)   { if (?x = 1, 1, ?x * st:fac(?x - 1)) } "
                + "function st:test(?x) { let(?y = ?x * ?x){ ?y} } ";
    ...
    public void myastpp() throws LoadException, EngineException {
        ...
        String init = "insert data {" + "<John> foaf:knows <Jim>, <Jack> " + "graph st:test " + "{"
        ...
        String init2 = "insert data {" + "<Jim>  foaf:knows <Jack>, <James> " + "<Jack> foaf:knows <Jesse>"
        ...
        String q = "select  * " + "where {" + "?x foaf:knows ?y " + "graph st:sys {" + "?y foaf:knows ?z, ?t "
                + "filter  exists { ?u rdfs:label 'toto' }" + "filter not exists { ?u rdfs:label 'tata' }" + "}"
                + "graph st:test { " + "?x rdfs:label ?n " + "filter exists { ?a rdfs:label 'John' }"
    ...
    public void myastpp2() throws LoadException, EngineException {
        ...
        String q = "template {" + "str(?res)" + "}" + "where {" + "graph st:sys {" + "bind (st:atw(st:turtle) as ?res)"
                + "}" + "}";
        Mappings map = exec.query(q);
    ...
    public void myastpp3() throws LoadException, EngineException {
        ...
        String q = "template {" + "str(?res)" + "}" + "where {" + "graph st:sys {" + "bind (st:atw('" + data
                + "template/test') as ?res)" + "}" + "}";
        Mappings map = exec.query(q);
        ////// System.out.println(map);
    ...
    public void testgraph() throws EngineException {
        ...
        String q2 = FOAF_PREF + "select * where {" + "?x foaf:age ?y" + "}";

        Mappings m2 = exec.query(q2);
        assertEquals(2, m2.size());

        String q3 = FOAF_PREF + "select * where {" + "?x foaf:pp* ?y" + "}";
    ...
    public void testeng() throws EngineException, LoadException {
        ...
        String q = "select * where {" + "graph eng:describe {"
                + "[] kg:index 0 ; kg:item [ rdf:predicate ?p ; rdf:value ?v ] " + "}" + "filter exists { ?x ?p ?y }"
                + "}";
        ...
        q = "select * where {" + "graph eng:query {" + "[] sp:predicate ?p " + "values ?p { rdf:predicate rdf:value} "
                + "}" + "}";
    ...
    public void testTr() throws EngineException {
        ...
        String init = "prefix ex: <http://example.org/>" + "insert data {" + "ex:C4 owl:unionOf (ex:C5 ex:C6) "
                + "ex:C0 owl:unionOf (ex:C2 ex:C3) " + "ex:C1 owl:unionOf (ex:C2 ex:C3) " + "}";

        exec.query(init);

        String q = "select *" + "  where { " + "    ?x owl:unionOf (?c1 ?c2)  ;" + "       owl:unionOf ?l" + "  }"
    ...
    public void testExists() throws EngineException {
        ...
        QueryProcess exec2 = QueryProcess.create(g2);
        String init1 = "insert data { " + "<John> rdfs:label 'John' " + "<James> rdfs:label 'James'" + "}";

        String init2 = "insert data { " + "<Jim> rdfs:label 'Jim' " + "}";

        String q = "select " + "(group_concat (exists {"

                + "select (group_concat(exists {" + "select (group_concat(exists {?x rdfs:label ?ll}) as ?temp) "
                + "where { ?x rdfs:label ?l } } ) as ?temp) " + "where {?x rdfs:label ?l}"
    ...
    public void testQQS() throws EngineException {
        ...
        String init = "insert data { " + "<John> rdfs:label 'John' " + "<James> rdfs:label 'James'" + "}";

        String q = "select * where {" + "graph ?g {" + "{" + "?x rdfs:label 'John' "
    ...
    public void testGCC() throws EngineException {
        ...
        String init = "insert data {" + "<John> rdf:value 'test'@fr, 'titi'@fr . "
                + "<Jack> rdf:value 'test'@fr,'titi'@en . " + "<Jim>  rdf:value 'test'@fr, 'titi' . " + "}";

        String q = "select ?x (group_concat(?v) as ?g) (datatype(?g) as ?dt)where {" + "?x rdf:value ?v" + "}"
    ...
    public void testPPOWL() throws EngineException, LoadException {
        ...
        String t1 = "prefix f: <http://example.com/owl/families/> " + "template  {  st:apply-templates-with(st:owl)}"
                + "where {}";

        String t2 = "prefix f: <http://example.com/owl/families/> " + "template  {  st:apply-templates-with(st:turtle)}"
                + "where {}";
    ...
    public void testPPSPIN() throws EngineException, LoadException {
        ...
        String t1 = "prefix f: <http://example.com/owl/families/> " + "template  {  st:apply-templates-with(st:spin)}"
                + "where {}";
    ...
    public void testMove1() throws EngineException {
        ...
        String init = "insert data {" + "graph <g1> {" + "<John> rdfs:label 'John' ." + "<James> rdfs:seeAlso <Jack> . "
                + "}" + "graph rdf: {" + "<John> rdfs:label 'John', 'Jim' ." + "<James> rdfs:seeAlso <Jack>, <Jim> . "
                + "}" + "graph <g3> {" + "<John> rdfs:label 'John' ." + "<James> rdfs:seeAlso <Jack> . " + "}" + "}";
    ...
    public void testJoinDistinct() {
        String init = "insert data {" + "<John> rdfs:label 'John', 'Jack' " + "}";

        String query = "select  distinct ?x where {" + "?x rdfs:label ?n " + "{?x rdfs:label ?a} "
                + "{?x rdfs:label ?b} " + "}";
    ...
    public void testLoadJSONLD() throws LoadException {
        ...
        String init = "select  " + "(count(*) as ?c)  " + " where {" + "?x ?p ?y" + "}";
    ...
    public void testJSONLD() throws LoadException, EngineException {
        ...
        String init = FOAF_PREF + "insert data {" + "foaf:knows rdfs:domain foaf:Person ; rdfs:range foaf:Person ."
                + "<John> foaf:knows <Jim> " + "<Jim> foaf:knows <James> " + "<Jack> foaf:knows <Jim> "
                + "<James> a foaf:Person" + "}";
    ...
    public void testDescr() {
        ...
        String q = "prefix ex: <http://test/> " + "prefix foaf: <http://foaf/> " + "describe ?z  where {"
    ...
    public void testRDFa() throws LoadException {
        ...
        String init = "prefix ex: <http://test/> " + "prefix foaf: <http://foaf/> " + "select  " + "(count(*) as ?c)  "
                + " where {" + "?x ?p ?y" + "}";
    ...
    public void testDT() throws EngineException {
        ...
        String init = "insert data { " + "[ rdf:value '2013-11-11'^^xsd:gYear " + ", '2013-11-11'^^xsd:gMonth "
                + ", '2013-11-11'^^xsd:gDay " + ", 'ar'^^xsd:int " + ", 'toto'^^xsd:double " + "" + "]}";
    ...
    public void testSystem() throws EngineException {
        ...
        String init = "insert data { graph kg:system { " + "kg:kgram kg:listen true " + "kg:store sp:query true "
                + "}}";

        String q = "select * where  {?x ?p ?y}";

        String query = "select ?res where {" + "graph kg:query {"
                + "select (st:apply-templates-with(st:spin, ?q) as ?res) where {" + "?q a sp:Select" + "}" + "}" + "}";
    ...
    public void testLoc2() throws EngineException, LoadException {
        ...
        String init = FOAF_PREF + "insert data { " + "[ foaf:knows <Jim> ] . " + "<Jim> foaf:knows <James> "
                + "<Jim> rdfs:label 'Jim' " + " " + "}";
        ...
        String q = FOAF_PREF + "select  *" + "where {" + "graph kg:system { " + "?a kg:version+ ?b " + "filter ("
                + "?a != ?a || " + "if (! (exists { ?a kg:date+ ?d } = false),  true, false)" + ")"
    ...
    public void testLoc() throws EngineException, LoadException {
        ...
        String init = FOAF_PREF + "insert data { " + "graph kg:system { " + "  kg:kgram kg:version '3.0.22' ;"
                + "    kg:date '2013-11-27'^^xsd:date ;" + "    kg:skolem true " + "}} ;" + "insert data { "
                + "[ foaf:knows <Jim> ]" + " " + "}";
        ...
        String q1 = "select *" + "where {" + "graph ?g { ?x ?p ?y  }" + "}";
        ...
        String q2 = "select *" + "where {" + "graph kg:system { ?x ?p ?y  }" + "}";
    ...
    public void testTurtle() throws EngineException {
        Graph g = createGraph();
        QueryProcess exec = QueryProcess.create(g);
        String init = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "insert data { " + "<John> foaf:name 'John' ; "
    ...
    public void testQV() {
        ...
        String init = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "insert data {"
                + "<John>  foaf:knows (<John> <Jim>)" + "}";

        String query = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "select * where {"
                + "?x foaf:knows (<Jim> ?x) " + "}" + "pragma {" + "kg:list kg:expand true" + "}" + "";
        try {
            exec.query(init);
    ...
    public void testQM() {
        ...
        QueryProcess exec = QueryProcess.create(g);
        String init = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "insert data {"
                + "<John> foaf:name 'John' ; foaf:age 18 ." + "<Jim> foaf:name 'Jim' ; foaf:knows <John> ." + "}";

        String query = "prefix sp: <http://spinrdf.org/sp#>" + "prefix foaf:    <http://xmlns.com/foaf/0.1/> "
                + "select * where {" + "?x foaf:name ?n " + "?x foaf:knows ?p " + "minus { ?x foaf:age ?a } "
                + "<James> foaf:fake ?f " + "?f a foaf:Person " + "?f sp:elements ?e " + "?f sp:test ?t "
                + "filter(?b >= 20)" + "}";
        try {
            exec.query(init);
    ...
    public void testSPIN() {
        ...
        String init = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "insert data {"
                + "<John> a foaf:Person ; foaf:knows <James> " + "<Jim> a foaf:Person " + "}";

        String query = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "select ?x (count(?y) as ?c) where {"
                + " { ?x a foaf:Test } union { ?x a foaf:Person ; foaf:pp* :: $path ?z }"
                + "optional { ?x foaf:knows ?y } " + "minus { ?x a foaf:Test } " + "filter(bound(?x) && ?x != 12)" + "}"
                + "group by ?x " + "having (?c >= 0)";
        try {
    ...
    public void testDistType() {
        ...
        String init = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "insert data {" + "<John> a foaf:Person " + "}";

        String query = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "select * where {" + " ?x a foaf:Person "
                + "}";
        try {
            e2.query(init);
    ...
    public void testDataset() {
        ...
        String init = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "insert data {" + "graph <http://inria.fr/g1> {"
                + "<John> foaf:name 'John' ; a foaf:Person" + "}" + "graph <http://inria.fr/g2> {"
                + "<Jim> foaf:name 'Jim' ; a foaf:Person" + "}" + "graph <http://inria.fr/o> {"
                + "foaf:Person rdfs:subClassOf foaf:Human" + "}" + "}";

        String query = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "select * " + "from named <http://inria.fr/g2>"
                + "where {" + "    {?x rdf:type foaf:Person ; ?p ?y}" + "}";

        String query2 = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "select * " + "from <http://inria.fr/g2>"
                + "where {" + " graph ?g   {?x rdf:type foaf:Person ; ?p ?y}" + "}";

        String update = "prefix foaf:    <http://xmlns.com/foaf/0.1/> " + "delete  where {?x ?p ?y}";
    ...
    public void TestOnto() {
        ...
        String init = "prefix c: <http://www.inria.fr/acacia/comma#>" + "insert data {"
                + "c:Human rdfs:subClassOf c:Animal " + "c:Man   rdfs:subClassOf c:Human "
                + "c:Woman rdfs:subClassOf c:Human " + "" + "<John> a c:Man " + "<Tigrou> a c:Cat "
                + "<Mary> a c:Woman " + "<James> a c:Human " + "}";

        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select * where {" + "?x a c:Human, ?t" + "}";
        try {
            exec.query(init);
            Mappings map = exec.query(query);
            //// System.out.println(map);
    ...
    public void testValues() {
        String init = "insert data {" + "<John> foaf:age '21'^^xsd:double " + "<Jack> foaf:age 21.0 " + "}";

        String query = "select  * where {" + "?x foaf:age ?a " + "?y foaf:age ?b " + "filter (?a = ?b && ?x != ?y) "
                + "}";

        String query2 = "select  * where {" + "{select (21.0 as ?a) where { }}" + "?x foaf:age ?a " + "}";

        String query3 = "select * where {" + "?x foaf:age ?a " + "}" + "values ?a { 21 21.0 }";
    ...
    public void testValues2() {
        String init = "prefix foaf: <http://xmlns.com/foaf/0.1/> " + "insert data {"
                + "<John>  foaf:name 'http://www.inria.fr' " + "<Jack>  foaf:name 'http://www.inria.fr' "
                + "<James> foaf:name <http://www.inria.fr> " + "<Jim>   foaf:name <http://www.inria.fr> "
                + "<John>  foaf:name 'http://www.inria.fr'@en " + "<Jack>  foaf:name 'http://www.inria.fr'@en " + "}";

        String query = "prefix foaf: <http://xmlns.com/foaf/0.1/> " + "select  * where {" + "?x foaf:name ?a "
                + "?y foaf:name ?a " + "filter(?x < ?y) " + "}";
    ...
    public void testMath() {
        ...
        String q = "prefix m: <http://ns.inria.fr/2013/math#>" + "template  { st:apply-templates-with(?p) }"
    ...
    public void testGC() {
        ...
        String init = "prefix ex: <http://example.org/> " + "insert data {" + "[ex:name 'John' , 'Jim']"
                + "[ex:name 'John' , 'Jim']" + "}" + "";

        String query2 = "prefix ex: <http://example.org/> "
                + "select (group_concat( concat(self(?n1), ?n2) ;  separator='; ') as ?t) where {" + "?x ex:name ?n1 "
                + "?y ex:name ?n2 " + "filter(?x != ?y)" + "" + "}";
    ...
    // @Test
    public void test1() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select check where {"
                + "?x rdf:type c:Person ;" + "c:FirstName 'John' ;" + "c:name ?n" + "}";
        QueryProcess exec = QueryProcess.create(graph);
        try {
            Mappings map = exec.query(query);
    ...
    public void test2() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>"
                + "select more * (kg:similarity() as ?sim) where {" + "?x rdf:type c:Engineer "
                + "?x c:hasCreated ?doc " + "?doc rdf:type c:WebPage" + "}" + "order by desc(?sim)";
        QueryProcess exec = QueryProcess.create(graph);
        try {
            Mappings map = exec.query(query);
    ...
    public void test2b() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>"
                ...
                + "?x rdf:type ?c1 filter(kg:similarity(?c1, c:Engineer) > .5) " + "?x c:hasCreated ?doc "
                + "?doc rdf:type ?c2 filter(kg:similarity(?c2, c:WebPage) > .4)" + "}" + "order by desc(?sim)";
        QueryProcess exec = QueryProcess.create(graph);
        try {
            Mappings map = exec.query(query);
    ...
    public void test5() {
        ...
        String update = "prefix c: <http://www.inria.fr/acacia/comma#>" + "insert data {"
                + "<John> c:name 'John' ; rdf:value (1 2 3)" + "c:name rdfs:domain c:Person "
                + "c:Person rdfs:subClassOf c:Human " + "}";

        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  *  where {"
                + "?x rdf:type c:Human ; c:name ?n ;" + "rdf:value @(1 2)" + "}";
    ...
    public void test8() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  *  where {" + "?x c:hasCreated ?doc"
                + "} " + "group by any " + "order by desc(count(?doc))" + "pragma {" + "kg:kgram kg:list true "
    ...
    public void test111() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select * (count(?doc) as ?c)"
                + "(kg:setProperty(?x, 0, ?c) as ?t)" + "where {" + "?x c:hasCreated ?doc" + "" + "}" + "group by ?x";

        String query2 = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select distinct ?x"
                + "(kg:getProperty(?x, 0) as ?v)" + "where {" + "?x c:hasCreated ?doc filter(kg:getProperty(?x, 0) > 0)"
    ...
    public void test14() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  *  where {"
                + "?x rdf:type c:Person; c:hasCreated ?doc " + "?doc rdf:type/rdfs:subClassOf* c:Document "
                + "c:Document rdfs:label ?l ;" + "rdfs:comment ?c" + "}";
    ...
    public void test16() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  * (st:number() as ?num)  where {"
                + "?x c:hasCreated ?doc " + "}";
    ...
    public void test18() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select * where {"
                + "c:Person rdfs:subClassOf+ :: $path ?c " + "graph $path {?a ?p ?b}" + "}";
    ...
    @Test
    public void test19() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select * "
                + "(xt:size($path) as ?l) (count(?a) as ?c) where {" + "?x c:isMemberOf+ :: $path ?org "
                + "graph $path {?a ?p ?b}" + "}" + "group by $path";
    ...
    public void test20() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>"
                ...
                + "select (ext:fun(?fn, ?ln) as ?res) where {" + "?x c:FirstName ?fn ; c:FamilyName ?ln" + "}";
    ...
    public void test21() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  * where {"
                + "?x c:FirstName 'Olivier' " + "filter(kg:contains('é', 'e')) " + "filter(kg:contains('e', 'é')) "
    ...
    public void test22() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  " + "where {"
                + "c:Engineer rdfs:subClassOf+ :: $path ?y " + "graph $path {?a ?p ?b}" + "}";
    ...
    public void test23() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  " + "where {"
                + "c:Engineer rdfs:subClassOf+ :: $path ?y " + "graph $path {{c:Toto ?p ?b} union {c:Engineer ?p ?b}}"
                + "}";
    ...
    public void test24() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  " + "where {"
                + "c:Engineer rdfs:subClassOf+ :: $path ?y " + "graph $path {?a ?p ?b filter(?a = c:Engineer)}" + "}";
    ...
    public void test28() {
        ...
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  " + "where {"
                + "c:Engineer rdfs:subClassOf+ :: $path ?y " + "graph $path {c:Engineer ?p ?b} "
    ...
    public void test30() {
        ...
        String query = "select  " + "(xt:size($path) as ?l) " + "(max(?l, groupBy(?x, ?y)) as ?m) "
                + "(max(?m) as ?max) " + "where {" + "?x rdfs:subClassOf+ :: $path ?y" + "}";
    ...
    @Test
    public void test31() {
        String query = "select (count(?l) as ?c1) " + "(count(distinct ?l) as ?c2) "
                + "(count(distinct self(?l)) as ?c3) " + "where {" + "?x rdfs:label ?l" + "}";
        QueryProcess exec = QueryProcess.create(graph);
        try {
            Mappings map = exec.query(query);
    ...
    public void test33() {
        ...
        String update = "insert data {" + "<John> foaf:knows <Jack> " + "<Jack> foaf:knows <Jim> " + "}";

        String query = "select * where {" + "?x foaf:knows+ :: $path <Jim> " + "graph $path { ?a foaf:knows ?b }" + "}";
    ...
    public void test34() {
        ...
        String query = "select * where {" + "?x ^ (foaf:knows+) :: $path <John> " + "graph $path { ?a foaf:knows ?b }"
                + "}";
    ...
    public void test36() {
        ...
        String update = "insert data {" + "<John> foaf:knows (<a> <b> <c>) " + "}";

        String query = "select * where {" + "graph ?g {optional{?x rdf:rest*/rdf:first ?y} " + "filter(!bound(?y))  "
                + "}" + "}";
    ...
    public void test38() {
        ...
        String init = "insert data {" + "<John>  <age> 20 " + "<John>  <age> 10 " + "<James> <age> 30 " + "}";

        String query = "select distinct (sum(?age) as ?s) where {" + "?x <age> ?age" + "}" + "group by ?x";
    ...
    public void test39() {
        ...
        String init = "insert data {" + "<a> foaf:knows <b> " + "<b> foaf:knows <a> " + "<b> foaf:knows <c> "
                + "<a> foaf:knows <c> " + "}";

        String query = "select * where {" + "<a> foaf:knows+ ?t " + "}";

        String query2 = "select * where {" + "<a> foaf:knows{1,10} ?t " + "}" + "pragma {kg:path kg:loop false}";
    ...
    public void test41() {
        ...
        String init = "prefix ex: <http://test/> " + "prefix foaf: <http://foaf/> " + "insert data {"
                + "ex:a foaf:knows ex:b " + "ex:b foaf:knows ex:c " + "ex:b rdfs:seeAlso ex:a "
                + "ex:c rdfs:seeAlso ex:b " + "}";

        String query = "prefix ex: <http://test/> " + "prefix foaf: <http://foaf/> " + "select * where {"
    ...
    public void test43() {
        ...
        String init = "prefix ex: <http://test/> " + "prefix foaf: <http://foaf/> " + "insert data {"
                + "ex:a foaf:knows ex:b " + "ex:b foaf:knows ex:e " + "ex:e foaf:knows ex:c "
                + "ex:b rdfs:seeAlso ex:a " + "ex:c rdfs:seeAlso ex:b " + "}";
    ...
    public void test47() {
        ...
        String init = "prefix i: <http://www.inria.fr/test/> " + "" + "insert data {" + "<doc> i:contain " + "'<doc>"
                + "<person><name>John</name><lname>K</lname><year>2000</year></person>"
                + "<person><name>James</name><lname>C</lname><year>1950</year></person>" + "</doc>'^^rdf:XMLLiteral   "
                + "}";

        String query = "" + "prefix i: <http://www.inria.fr/test/> " + "construct {" + "[i:name ?name]" + "} where {"
                + "select (concat(xt:objectvalue(?n), '.', xt:objectvalue(?l)) as ?name) "
                + "(xsd:integer(xt:objectvalue(?y)) as ?yy) where {" + "?x i:contain ?xml "
    ...
    public void test49() {
        ...
        String query = "" + "base      <http://www.example.org/schema/>" + "prefix s: <http://www.example.org/schema/>"
                + "prefix i: <http://www.inria.fr/test/> " + "construct {?su ?pr ?ob} " + "where {" + "select * where {"
                ...
                + "bind  (uri(xt:objectvalue(?s)) as ?su) " + "bind  (uri(xt:objectvalue(?p)) as ?pr)   "
    ...
    public void test50() {
        ...
        String init = "prefix : <http://example.org/> " + "" + "insert data {" + ":A0 :P :A1, :A2 . "
                + ":A1 :P :A0, :A2 . " + ":A2 :P :A0, :A1" + "}";

        String query = "prefix : <http://example.org/>" + "select * where { :A0 ((:P)*)* ?X }";
    ...
    public void test51() {
        ...
        String query = "prefix : <http://example.org/>" + "select * where { ?X ((:P)*)* :A1 }";
    ...
    public void test52() {
        ...
        String init = "prefix : <http://example.org/> " + "" + "insert data {" + ":a :p :b, :c ." + ":b :q :d "
                + ":c :q :d " + ":d :p :e " + ":e :q :f " + "" + "} ";

        String query = "prefix : <http://example.org/>" + "select * where { :a (:p/:q)+ ?y }";
    ...
    @Test
    public void test53() {
        String query = "prefix c: <http://www.inria.fr/acacia/comma#>" + "select  * (kg:similarity() as ?sim) where {"
                + "?x rdf:type c:Engineer " + "}" + "order by desc(?sim)" + "pragma {kg:match kg:mode 'strict'}";
    ...
    @Test
    public void test54() {
        String query = "select * where {" + "graph ?g {?s <name> ?o} " + "?s <age> ?a" + "}";
        Graph graph = createGraph();
    ...
    /**
     * Two graphs with partial ontology each Each graph answer with its viewpoint on
     * the ontology
     */
    @Test
    public void test55() {

        String o1 = "prefix foaf: <http://foaf.org/>" + "insert data {" + "foaf:Human rdfs:subClassOf foaf:Person "
                + "}";

        String o2 = "prefix foaf: <http://foaf.org/>" + "insert data {" + "foaf:Man rdfs:subClassOf foaf:Person " + "}";

        String init1 = "prefix foaf: <http://foaf.org/>" + "insert data {" + "<John> a foaf:Human" + "}";

        String init2 = "prefix foaf: <http://foaf.org/>" + "insert data {" + "<Jack> a foaf:Man" + "}";

        String query = "prefix foaf: <http://foaf.org/>" + "select * where {" + "?x a foaf:Person" + "}";
    ...
    public void test56() {
        ...
        String init = "insert data {" + "graph <g1> {<John> foaf:knows <Jim> }" + "graph <g2> {<Jim> foaf:knows <Jack>}"
                + "}";

        String query = "select  * where {" + "?x foaf:knows+ ?y " + "filter(?y = <Jack> || <John> = ?x)" + "}";
    ...
    public void test58() {
        ...
        String init = "insert data {" + "<John> foaf:knows <Jack> " + "<John> foaf:knows <Jim> "
                + "<Jim> foaf:knows <Jack> " + "}";

        String query = "select * (xt:size($path) as ?l) where {" + "?x (foaf:knows|rdfs:seeAlso)+ :: $path ?y" + "}";
    ...
    public void test59() {
        ...
        String init = "insert data {" + "<John> foaf:knows <Jack> " + "<John> foaf:name 'Jack' ;" + "foaf:age 12 ;"
                + "foaf:date '2012-04-01'^^xsd:date ;" + "foaf:knows [] " + "}";
    ...
    public void test63() {
        ...
        String init = "prefix foaf: <http://xmlns.com/foaf/0.1/>" + "insert data {"
                + "<A> foaf:date '2012-05-10'^^xsd:dateTime " + "<B> foaf:date '2012-05-11'^^xsd:dateTime "
        ...
        String query = "prefix foaf: <http://xmlns.com/foaf/0.1/>" + "select * where {" + "?x foaf:date ?date" + "}"
                + "order by desc(?date)";
    ...
    public void test65() {
        ...
        String query = "select * where {" + "?x ?p ?y . ?z ?q ?y filter(?x != ?z)" + "}";
    ...
    public void testRelax() {
        ...
        String init = "prefix foaf: <http://xmlns.com/foaf/0.1/> " + "prefix c: <http://www.inria.fr/acacia/comma#>"
                + "insert data {" + "<John> foaf:type c:Researcher " + "<John> foaf:knows <Jack> "
                + "<Jack> foaf:type c:Engineer " + "<John> foaf:knows <Jim> " + "<Jim> foaf:type c:Fireman "
                + "<e> foaf:type c:Event " + "}";

        String query = "prefix foaf: <http://xmlns.com/foaf/0.1/> " + "prefix c: <http://www.inria.fr/acacia/comma#>"
                + "select   more * (kg:similarity() as ?s) where {" + "?x foaf:type c:Engineer " + "?x foaf:knows ?y "
                + "?y foaf:type c:Engineer" + "}" + "order by desc(?s) " + "pragma {kg:kgram kg:relax (foaf:type)}";
    ...
    public void testQueryGraph() {
        ...
        String cons = "prefix : <http://example.org/> " + "" + "construct {?x :p []}" + "where {?x :p ?y}";

        String init2 = "prefix : <http://example.org/> " + "" + "insert data {" + ":a :p [] ." + "}";
    ...
    public void testOption() {
        ...
        String init = "prefix : <http://example.org/> " + "" + "insert data {" + ":a :p :b, :c ." + ":b :p :d, :a "
                + ":c :p :d " + "" + ":e :p :b, :c ." + "" + "} ";

        String query = "prefix : <http://example.org/> " + "select *  where  {" + "?x ((:p/:p) ?)  ?y " + "}";
    ...
    public void testWF() {
        ...
        String init = "prefix c: <http://example.org/>" + "insert data {" + "[ c:hasParent [] ]" + "}";

        String update = "prefix c: <http://example.org/>" + "insert {?y c:hasChild ?x}" + "where { ?x c:hasParent ?y}";
    ...
    public void testCompile() throws EngineException {
        ...
        String query = "select * where {" + "graph ?g {?x ?p ?y " + "{select * where {"
                + "?a (rdf:type@[a rdfs:Resource]) :: $path ?b  " + "{values ?a {<John>}}" + "}" + "order by ?a "
                + "group by ?b " + "having (?a > ?b) " + "}" + "?a (rdf:type@[a rdfs:Resource]) ?b" + "" + "}" + "}";
        Mappings map = exec.query(query);
