...
Corese offers a range of storage systems for storing and managing your RDF data. In this document, you'll find information on how to use these storage systems with Corese, including instructions on configuring and utilizing them in the GUI, Server, and Library.

## 1. Introduction

In Corese versions prior to 4.4, graphs were only loaded and manipulated in RAM. With the introduction of Data Managers in version 4.4, the Corese engine and storage systems are now fully decoupled. This offers several advantages such as :
...
A data manager in Corese is a bridge between the Corese engine and an storage systems, enabling the engine to store and manage graph data in a variety of storage systems while abstracting away the underlying details of those systems.
...
- `Rdf4jModelDataManager` for RDF4J models
- `CoreseGraphDataManager` for Corese graphs
...
TDB1 is a persistent storage system that supports transactions and concurrent access. In our larger tests, it has been able to efficiently load and manage graphs with up to 600 million triples. However, it is likely capable of handling even larger graph sizes.
...
> You can use all the native tools and [Apache Jena - TDB Command-line Utilities](https://jena.apache.org/documentation/tdb/commands.html) . For example, you can use `tdbloader` to efficiently create a TDB1 database from serialized RDF data.
...
However, Corese Graph does not offer persistence and only supports concurrent access.
...
This implementation is not optimized for storing large amounts of data and does not support persistence, concurrency, or transactions. It was our first implementation as a proof-of-concept and is not recommended for use in production environments. However, it may still be useful for small-scale testing and development purposes.
...
To run Corese-GUI or Corese-Server with a property file, the `-init` option can be used. For instance, the following bash command runs Corese-GUI using the `gui.properties` file:
...
- `PARAM_BD1`: (Optional) Constructor parameters for the storage system.

| DB type | Options |
| --- | --- |
| jena_tdb1 | nothing  (data load in memory) \\| path of TDB1 database |
| rdf4j_model | nothing |
| corese_graph | nothing |
...
> There are two ways to execute SPARQL queries in Corese:
> 
> - In the GUI: To execute a query in the GUI, open the Corese GUI and enter the query in the SPARQL Query tab.
>   
> - In the Server: To execute a query via an endpoint, send a request to the endpoint `http://localhost:8080/sparql`.
>   

### 4.3. [Optional] Assign storage to a specific SPARQL endpoint with Corese Server

Corese Server allows you to assign storage to a specific SPARQL endpoint by creating a properties file (eg: `server.properties`) and a profile file (eg: `profil.ttl`). The properties file defines the storage systems available and their locations, while the profile file assigns a storage system to a specific endpoint.
...
With this configuration, the endpoint `<http://localhost:8080/album/sparql>` will contain `db1` data, and `<http://localhost:8080/music/sparql>` will contain `db2` data.

To start the server with these configurations, run the following command:

```bash
java -jar corese-server.jar -init "server.properties" -pp "profil.ttl"
```
