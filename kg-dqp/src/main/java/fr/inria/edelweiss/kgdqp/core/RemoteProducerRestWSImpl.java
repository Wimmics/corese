...
/**
 * Implementation of the remote producer that conforms to SPARQL 1.1 GraphStore protocol.
 *
 * @author Eric TOGUEM, eric.toguem@uy1.uninet.cm
 */
@Deprecated
public class RemoteProducerRestWSImpl implements Producer {

    private static Logger logger = Logger.getLogger(RemoteProducerRestWSImpl.class);
    private SPARQLRestEndpointClient rp;
    private HashMap<String, Boolean> cacheIndex = new HashMap<String, Boolean>();

    public RemoteProducerRestWSImpl(URL url) {
        rp = new SPARQLRestEndpointClient(url.toString());
    }
    ...
    public Iterable<Entity> getEdges(Node gNode, List<Node> from, Edge qEdge, Environment env) {
        ...
        try {
            if (true) {
                String sparqlRes = rp.doGet(query);
    ...
    public Iterable<Entity> getNodes(Node gNode, List<Node> from, Edge qEdge, Environment env, List<Regex> exp,
        ...
        try {
            ...
            sw.start();
            String sparqlRes = rp.doPost(sparql);
            sw.reset();
            sw.start();
            if (sparqlRes != null) {
                Load l = Load.create(g);
    ...
    public SPARQLRestEndpointClient getRp() {
        return rp;
    }
