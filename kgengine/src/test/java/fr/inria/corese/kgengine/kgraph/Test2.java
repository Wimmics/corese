...
import fr.inria.corese.sparql.cg.datatype.CoreseBoolean;
...
public class Test2 {
	...
	void process(){
		...
		String ndata = "/home/corby/Download/Cytoscaphe/GeneRifs_Annotations/";
...
//loader.load(data + "ign/carto2.owl");
//loader.load(data + "ign/carto.rul");
//
//RuleEngine re = loader.getRuleEngine();
//re.setDebug(true);
//re.process();


query = 	
"prefix i: <http://www.semanticweb.org/ontologies/2008/11/Ontology1228129138638.owl#> "+
"select * where  {"+
"?p i:aPourVariableVisuelleTeinte ?y " +
//"?p rdf:type i:Caisson" +
"}";

query = "select  (uri(concat('alt', (kg:number()))) as ?n) ?m where {" +
"?x ?p ?y" +
"{select (uri(concat('sol', (kg:number()))) as ?m) where {?z ?q ?t} limit 6}" +
"} group by ?x limit 5";
...
//QueryLoad ql   = QueryLoad.create(qe);
//qe.setDebug(true);
//
//ql.load(data + "kgraph/query.rq");
//ql.load(data + "kgraph/query2.rq");
//ql.load(data + "kgraph/query.rq");


//loader.load(data + "kgraph/tmp.rdf");

query = "select  ?q ?t where {" +
...
"c:Document (^rdfs:range/rdfs:domain){1,6} :: $path ?any " +
//"graph $path {?a ?p ?b}" +
"}";

//loader.setPattern("_bioAnnot");
//loader.load(ndata + "NM_002391_bioAnnot.rdf");
//loader.load(ndata + "NM_002391.rdf");
//BasicConfigurator.configure();
//loader.load(root + "design/pipetest.rdf");
//loader.setDebug(true);
//loader.load(root + "design/query");
//loader.setBuild(new BuildPlugin(graph));
//loader.load("../kgengine/tmp.rdf");

//BuildOptim bb = BuildOptim.create(graph);
////bb.define(name, cl);
//loader.setBuild(bb);
//loader.load(data + "cstb/alban.rdf");

QueryEngine qe = loader.getQueryEngine();
//qe.process();
...
query = "insert data {<a> <p> <b>  <a> <q> <c>}";
 ...
 //loader.load(root + "comma/comma.rdfs");
 try {
	loader.loadWE(root + "comma/comma.rdfs");
	loader.loadWE(root + "comma/model.rdf");
 ...
 //query = ql.read(data + "kgraph/path.txt");
// query = "prefix ext: <function://test.kgraph.PluginImpl> " +
// 		"construct {?x rdf:first ?val ; rdf:rest ?y} " +
// 		"where {" +
// 			"{select (count(*) as ?count) where {?a ?p ?val filter(isLiteral(?val)) }} . " +
//	 		"{select * " +
//	 		"(bnode(concat('_:b', kg:number())) as ?x) " +
//	 		"(if ((kg:number()+1 = ?count), " +
//	 			"rdf:nil, " +
//	 			"bnode(concat('_:b', kg:number()+1))) as ?y) " +
//	 		"where {?a ?p ?val  filter(isLiteral(?val))" +
//	 		"filter( ext:test(?a, kg:env()) )" +
//	 		"}} " +
//	 	"} " +
// 		"";
// 
...
try {
	...
	Interpreter inter = (Interpreter) exec.getEvaluator();
	inter.getProxy().setPlugin(new PluginImpl());
	...
	//for (int i = 0; i<10; i++)
		...
		//lMap = exec.query(query2);

		RDFFormat f = RDFFormat.create(exec.getGraph(lMap));
		//System.out.println(f);
	
	t2 = new Date().getTime();

	XMLFormat ff = XMLFormat.create(lMap);
	System.out.println(lMap);
	
	//URL uri = new URL("scheme://test.fr");
	//URL ur2 = new URL("gogo");
	//System.out.println(new URL("mailto:toto"));
//	System.out.println(new URL("ftp:toto"));
//	System.out.println(new URL("file://uuu"));
//	System.out.println(new URL("http://uuu"));
	
//	query = "base <http://www.inria.fr> " +
//			"prefix ext: <function://fr.inria.Test>" +
//			"select (ext:fun() as ?test) where {}";
//	
//	Mappings map = exec.query(query);
//
//	System.out.println(map.getQuery().getAST());
	
//	String pname = "scheme://test.fr";
//	System.out.println("** Match " + pname.matches("[a-z]*://.*"));
	
			
			
//	XMLResult r = XMLResult.create(ProducerImpl.create(graph));
//	Mappings m = r.parse(ff.toString());	
//	System.out.println(m);
	
	//CSVFormat cf = CSVFormat.create(lMap);
	TSVFormat cf = TSVFormat.create(lMap);
	//System.out.println(cf);
	
	//System.out.println(cf.escape("a\"bb\"ccc\""));
