...
public class MainFrame extends JFrame implements ActionListener{
	...
	private static final String TITLE = "Corese/KGRAM 3.0 - Wimmics Inria I3S - 2013-09-30";
...
//					 "c http://www.inria.fr/acacia/comma# ");
				...
				new ChangeListener(){
					...
					{					
						...
						if(c instanceof MyJPanelQuery){
					    	...
					    	else temp.getButtonTKgram().setEnabled(false);
	...
	public void appendMsg(String msg){
	    ...
	    try {
	    	doc.insertString(ongletListener.getTextPaneLogs().getDocument().getLength(), msg + "\n\n", null);
	...
	public void newQuery() {
		 ...
		 * Juste après la création de l'onglet Query il y a donc 3 composants au conteneur d'onglet (Listener, Query, +)
		 * On différencie si c'est le 1er onglet créé ou non 
		 * car le fait d'ajouter le croix fermante à l'onglet ajoute un composant au conteneur d'onglet
		 * (1 onglets = 1 composants onglet + 1 composant "croix fermante" = 2 composants)
		 * mais ceci une seule fois (2 onglets = 2 composants onglet + 1 composant "croix fermante" = 3 composants)
		 * initTabComponent(0); appliquerait la croix fermante au 1er onglet du conteneur cad à "Listener"
		 * initTabComponent(conteneurOnglets.getComponentCount()-1); l'appliquerait au dernier composant du conteneur cad à "+"
		 * initTabComponent(conteneurOnglets.getComponentCount()-3); car il faut retirer la croix et l'onglet "+" dans le compte
		 */
		...
		else initTabComponent(conteneurOnglets.getComponentCount()-3);
...
//		}
	...
	/**
	 * Permet d'ordonnée une liste en utilisant la méthode SortArray
	 * Récupère le model de la liste afin de réorganiser celle-ci 
	 */
	public  void sort(){
	...
	/**
	 * Permet de charger tout type de fichier mais en maintenant un ordre de chargement
	 * C'est a dire qu'il recharge la liste ordonnée apèrs chargement de nouveau fichier pour maintenir la cohérence
	 */
	public void loadFile(){
		try {
			...
			for(int i=0;i<getOngletListener().getModel().getSize();i++){
				myCorese.load(ongletListener.getModel().get(i).toString());
			}
			
    		} catch (EngineException e1) {
    			logger.fatal("Attempted to read file:",e1);
    			appendMsg("ERROR.\n" + myCapturer.getContent());
    		}
		}
	...
	public void loadRDFs(){	
		...
		JFileChooser fileChooser = new JFileChooser(l_path_courant);
		fileChooser.setMultiSelectionEnabled(true);
		fileChooser.addChoosableFileFilter(FilterRDFS);
        int returnValue = fileChooser.showOpenDialog(null);
        if (returnValue == JFileChooser.APPROVE_OPTION) {	
        	File l_Files[] = fileChooser.getSelectedFiles();
	        for(File f :l_Files){
	        	l_path = f.getAbsolutePath();
	        	l_path_courant = f.getParent();   //recupere le dossier parent du fichier que l'on charger	        
		        if (l_path != null) {
					getOngletListener().getModel().addElement(l_path);		//permet d'ajouter tous les fichiers sélectionnés dans la liste afin de faciliter le travail de la méthode loadFile()
	...
	public void loadRDFs(String fichier){
		try {
        ...
        if (returnValue == JFileChooser.APPROVE_OPTION) {
        	...
        	for(File f : l_Files)
	        {
	        	...
	        	l_path_courant = f.getParent();   //recupere le dossier parent du fichier que l'on charger
	        	 ...
	        	 if (l_path != null) {
						getOngletListener().getModel().addElement(l_path);
						appendMsg("Loading "+extension(l_path)+" File from path : " +l_path+"\n Done.");
...
//		}
	...
	void set(int event){
		if (buffer!=null) buffer.set(event);
	}
	...
	public static void main(String[] p_args){
	    ...
	    if(p_args.length>0) coreseFrame = new MainFrame(aCapturer, p_args[0]);
	    else 
