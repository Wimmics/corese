 ...
 * Compute graph homomorphism and (extended) SPARQL
 * Use:
 *  a Stack of expression Exp
 *  a Memory for Node/Edge bindings
 *  an abstract Producer of candidate Node/Edge
 *  an abstract Evaluator of Filter
 *  an abstract Matcher of Node/Edge
 *  
 *  - path statement is an EDGE with a boolean isPath
 *  this edge needs an Edge Node (a property variable)
 *   
 *  
 *  TODO:
 *  optimize: query ordering,  search by dichotomy (in a cache)
 * 
...
public class Eval implements  ExpType, Plugin {
	 ...
	 * @param e filter evaluator, given an environment (access to variable binding)
	 */
	...
	/**
	 * Eval KGRAM query and subquery
	 * For subquery, this eval is a copy which shares the memory with outer eval
	 */
	...
	/**
	 * Subquery processed by a function call that return Mappings
	 * Producer may cast the result into Mappings
	 * use case:
	 * {select xpath(?x, '/book/title') as ?val where {}}
	 * Mappings may be completed by filter (e.g. for casting)
	...
	/**
	 * additional filter of functional
	 * select xpath() as ?val
	...
	void debug(){
		...
		if (results.size()==0){
			...
			else Message.log(Message.FAIL_AT , maxExp); 
		}
	}
	
	/**
	 * 
	 * Copy current evaluator to eval subquery
	 * same memory (share bindings)
	 * new exp stack
	 */
	Eval copy(){
	 ...
	 * copy sub query select variables that are already bound in current memory
	 * 
	 */
	...
	private Memory copyMemory(Memory memory, Query query, Query sub){
		...
		if (hasEvent) memory.setEventManager(manager);
		return mem;
	}
	
	/**
	 * copy of Memory may be stored in exp.
	 * Reuse data structure after cleaning and init 
	...
	private	void starter(Query q){
		...
		if (initialResults!=null) results = initialResults;
		else results =  Mappings.create(query);
		if (hasEvent) results.setEventManager(manager);
	...
	/**
	 * What should be done before throw LimitException
	 * - close path threads if any
	 */
	private void clean(){
...
//			}
	 ...
	 * 
	 * AND pushes the elements in the stack
	 * UNION generates two copies of the stack that are run in sequence,
	 * one for each arg of the union
	 * OPTION{EXP}  generates  WATCH EXP CONTINUE
	 * OPTION{A UNION B} generates two stacks that share the same CONTINUE
	 * if one of them succeeds, the option succeeds
	 * GRAPH NODE EXP evaluate the EXP 
	 * and relations must be quads that contain a graph node
	 * NODE enumerate and bind a node
	 * NODE may also bind NODEs given in the args
	 * EDGE enumerate relations
	 * FILTER evaluate a filter
	 * 
	 * QUERY implements select subquery, share bindings
	 * NOT { EXP } implements NAF
	 * compiled as WATCH EXP BACKJUMP
	 * FORALL {}{}
	 * EXIST {}
	 * 
	 * Manage backjump, i.e. backtrack at lever less than n-1
	 * needed for NOT in order to backtrack at once before the not
	 * 
	 * 
	 * 
	 */
...
private	int eval(Producer p, Node gNode, Stack stack, int n, boolean option)  {
		...
		if (exp.isFail()){
			...
			case OPTION:
				...
				// compiled as: WATCH EXP CONTINUE
...
//					}
//					else {
//						backtrack = option(gNode, exp, stack, n, true);
//					}
//				}
//				else 
				{
			...
			case NOT:
				/**
				 * compiled as: WATCH EXP BACKJUMP
				 * true means if reach BACKJUMP, WATCH must backtrack after
				 * negation fail
				 * 
				 */
...
private	int cbind(Producer p, Node gNode, Exp exp, Stack stack,  int n, boolean option){
	...
	/**
	 * Bind graph node in new memory if it is bound in current memory
	 * use case:
	...
	/**
	 * {?x c:name ?name} minus {?x c:name 'John'}
	 * TODO: 
	...
	/**
	 * exp is graph ?g { PAT }
	 * set GRAPHNODE(?g) before and GRAPHNODE(?gNode) after
	 */
	...
	/**
	 * exp is (1) GRAPHNODE(nextGraph) or  (2) option|not
	 * (1) gNode is not bound, enumerate graph names here
	 * then set gNode as nextGraph
	 * use case : graph ?gNode { optional { }} . GRAPHNODE(nextGraph)
	 * next is n+1
	 * (2) exp = option{} | not{}
	 * gNode is not bound, enumerate graph nodes and bind gNode
	 * use case: graph ?gNode { not {} } 
	 * use case: graph ?gNode { optional {?x p ?y } !bound(?y) ?z q ?t} 
	 * next is n
	 */
	...
	private	int filter(Producer p, Node gNode, Exp exp, Stack stack, int n,  boolean option){
		...
		if (success){
			...
			if (exp.getNode() != null){
				...
				// filter fail: skip preceding edge
				// see exp.edgeFilter()
				backtrack = n-2;
			}
		}
			  ...
			  * draft to be removed
			  * use case:
			  * x p y . z q t . filter(z < x)
			  */
			//if (n >= 2 && query.isTest()) backtrack = n-2;
		}
	...
	/**
	 * graph $path { ?a ?p ?b }
	 * producer is temporarily replaced by path
	...
	private	int inPath3(Node gNode, Exp exp, Stack stack, int n,  boolean option){
		...
		// path serve as edge producer in new(eval, memory, stack):
		Eval pEval = copy(getMemory(exp), env.getPath(qPath), evaluator);

		Mappings pMap = pEval.subEval(query, gNode, Stack.create(exp.rest()), 0);

		for (Mapping map : pMap){
			boolean success = env.push(map, n);
			if (isEvent){
				send(evENUM, exp, map, success);
			}
			if (success){
...
//			}
	...
	/**
	 * select * where {{select distinct ?y where {?x p ?y}} .  ?y q ?z}
	 * new eval, new memory, share only sub query select variables
	 * 
	 */
	private	int query(Node gNode, Exp exp, Stack stack, int n, boolean option){
		int backtrack = n-1, evENUM = Event.ENUM;
		boolean isEvent = hasEvent;
	...
	/**
	 * exp.first() is a subquery that implements a BIND() 
	 * pop the binding at the end of group pattern
	 */
...
//						}
	...
	/**
	 * In case of backjump
	 * Some node must differ between previous and current
	 * A node that is common to qEdge and qPast
	 */
	...
	/**
	 * res is a result of sub query
	 * bind the select nodes of sub query into current memory
	 * retrieve outer node that correspond to sub node 
	 * 
	 */
	...
	private	boolean match(Edge qEdge, Edge edge, Node gNode, Node graphNode, Memory memory){
		if (! match.match(qEdge, edge, memory)) return false;
		if (gNode == null) return  true;
	...
	private	boolean match(Node qNode, Node node, Node gNode, Node graphNode){
		Memory env = memory;
		if (! match.match(qNode, node, env)) return false;
		if (gNode == null) return  true;
