...
//            e.setBGP(true);
	...
	/**
	 * Add a bind exp at the beginning this exp qnode will be bound with
	 * node in the stack before processing this exp
	 */
	...
	/**
	 * use case: select distinct ?x where add an ACCEPT ?x statement to
	 * check that ?x is new
	 */
	...
	/**
	 * check special case: e1: ?x path ?y e2: graph path {} e2 cannot be
	 * moved before e1
	 */
	 ...
	 * @param expVar: list of variables bound by expressions Add in expVar
	 * the variables bound by this expression that are in filterVar bound
	 * means no optional, no union
	 */
	...
	/**
	 * Return variable nodes of this exp use case: find the variables for
	 * select * PRAGMA: subquery : return only the nodes of the select
	 * return only variables (no cst, no blanks) minus: return only nodes of
	 * first argument
	 */
	...
	/**
	 * select * return nodes that (may) bind variables, do not return exists
	 * and minus nodes
	 */
		...
		for (Node qNode : lSelNode) {
			if (!lNode.contains(qNode)) {
				if (lExistNode.contains(qNode)) {
					/**
					 * use case: select * where { {select *
					 * where {?x rdf:rest* /rdf:first ?y}}
					 * filter(! exists{?x rdf:first ?y}) }
					 * lNode = {} lSelNode = {?x, ?y}
					 * lExistNode = {?x, ?y} The result of
					 * sub query is bound to exists nodes in
					 * order to join
					 */
	...
	/**
	 * compute the variable list use case: filter(exists {?x ?p ?y}) no
	 * minus
	 */
